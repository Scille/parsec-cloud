...
#[allow(dead_code)]
fn variant_client_editics_join_session_error_rs_to_js(
    rs_obj: libparsec::ClientEditicsJoinSessionError,
) -> Result<JsValue, JsValue> {
    let js_obj = Object::new().into();
    let js_display = &rs_obj.to_string();
    Reflect::set(&js_obj, &"error".into(), &js_display.into())?;
    match rs_obj {
        libparsec::ClientEditicsJoinSessionError::Internal { .. } => {
            Reflect::set(
                &js_obj,
                &"tag".into(),
                &"ClientEditicsJoinSessionErrorInternal".into(),
            )?;
        }
        libparsec::ClientEditicsJoinSessionError::Offline { .. } => {
            Reflect::set(
                &js_obj,
                &"tag".into(),
                &"ClientEditicsJoinSessionErrorOffline".into(),
            )?;
        }
    }
    Ok(js_obj)
...
// client_editics_join_session
#[allow(non_snake_case)]
#[wasm_bindgen]
pub fn clientEditicsJoinSession(
    client_handle: u32,
    workspace_id: String,
    file_id: String,
) -> Promise {
    future_to_promise(libparsec::WithTaskIDFuture::from(async move {
        ...
        let ret =
            libparsec::client_editics_join_session(client_handle, workspace_id, file_id).await;
        Ok(match ret {
            Ok(value) => {
                let js_obj = Object::new().into();
                Reflect::set(&js_obj, &"ok".into(), &true.into())?;
            ...
            Err(err) => {
                let js_obj = Object::new().into();
                Reflect::set(&js_obj, &"ok".into(), &false.into())?;
                let js_err = variant_client_editics_join_session_error_rs_to_js(err)?;
                Reflect::set(&js_obj, &"error".into(), &js_err)?;
                js_obj
            }
        })
