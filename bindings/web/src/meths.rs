...
fn struct_server_config_js_to_rs(obj: JsValue) -> Result<libparsec::ServerConfig, JsValue> {
    ...
    let openbao = {
        ...
        if js_val.is_null() {
            ...
            Some(variant_open_bao_config_js_to_rs(js_val)?)
        }
    };
...
fn struct_server_config_rs_to_js(rs_obj: libparsec::ServerConfig) -> Result<JsValue, JsValue> {
    ...
    let js_openbao = match rs_obj.openbao {
        Some(val) => variant_open_bao_config_rs_to_js(val)?,
        None => JsValue::NULL,
    };
...
fn variant_available_device_type_js_to_rs(
    ...
    match tag.as_str() {
        ...
        "AvailableDeviceTypeOpenBao" => {
            let openbao_preferred_auth = {
                let js_val = Reflect::get(&obj, &"openbaoPreferredAuth".into())?;
                js_val
                    .dyn_into::<JsString>()
                    .ok()
                    .and_then(|s| s.as_string())
...
fn variant_available_device_type_rs_to_js(
    ...
    match rs_obj {
        ...
        libparsec::AvailableDeviceType::OpenBao {
            ...
            let js_openbao_preferred_auth = openbao_preferred_auth.into();
            Reflect::set(
                &js_obj,
                &"openbaoPreferredAuth".into(),
                &js_openbao_preferred_auth,
            )?;
...
#[allow(dead_code)]
fn variant_open_bao_config_js_to_rs(obj: JsValue) -> Result<libparsec::OpenBaoConfig, JsValue> {
    let tag = Reflect::get(&obj, &"tag".into())?;
    let tag = tag
        .as_string()
        .ok_or_else(|| JsValue::from(TypeError::new("tag isn't a string")))?;
    match tag.as_str() {
        _ => Err(JsValue::from(TypeError::new(
            "Object is not a OpenBaoConfig",
        ))),
    }
}

#[allow(dead_code)]
fn variant_open_bao_config_rs_to_js(rs_obj: libparsec::OpenBaoConfig) -> Result<JsValue, JsValue> {
    let js_obj = Object::new().into();
    match rs_obj {}
    Ok(js_obj)
}
