...
msTest('Recover device using shamir', async ({ context }) => {
  ...
  const page = (await context.newPage()) as MsPage;
  await setupNewPage(page, { enableShamir: true, testbedTemplate: 'shamir' });
  ...
  await expect(claimModal.locator('.recipient').nth(0).locator('.button-start')).toBeHidden();
  await expect(claimModal.locator('.recipient').nth(0).locator('.spinner')).toBeVisible();
  await expect(claimModal.locator('.recipient').nth(0).locator('.cancel-button')).toBeVisible();

  // Cancel and restart
  await claimModal.locator('.recipient').nth(0).locator('.cancel-button').click();
  await expect(claimModal.locator('.recipient').nth(0).locator('.button-start')).toBeVisible();
  await expect(claimModal.locator('.recipient').nth(0).locator('.spinner')).toBeHidden();
  await expect(claimModal.locator('.recipient').nth(0).locator('.cancel-button')).toBeHidden();
...
msTest('Claimer cancel shamir device recovery', async ({ context }) => {
  ...
  await claimModal.locator('.closeBtn').click();
  await answerQuestion(page, true, {
    expectedQuestionText: 'ARE YOU SURE?',
    expectedTitleText: 'INTERRUPT THE PROCESS',
    expectedNegativeText: 'KEEP GOING',
    expectedPositiveText: 'INTERRUPT',
  });
  await expect(claimModal).toBeHidden();
  await expect(page).toBeHomePage();
  await expect(greeter1Modal).toBeHidden();
  await expect(greeter1Page).toShowToast('CLAIMER CANCELLED', 'Error');
});

msTest('Greeter cancel shamir device recovery', async ({ context }) => {
  msTest.setTimeout(60_000);
  const SAS_CODE_RE = /^[A-Z0-9]{4}$/;
  ...
  await exch1Modal.locator('.closeBtn').click();
  await answerQuestion(greeter1Page, true, {
    expectedQuestionText: 'ARE YOU SURE?',
    expectedTitleText: 'INTERRUPT THE PROCESS',
    expectedNegativeText: 'KEEP GOING',
    expectedPositiveText: 'INTERRUPT',
  ...
  await expect(exch1Modal).toBeHidden();
  await expect(page).toShowToast('GREETER CANCELLED', 'Error');

  // Modal goes back to its initial state
  await expect(claimModal.locator('.parts')).toHaveText('PARTS: 0/2');
  await expect(claimModal.locator('.recipient-info')).toHaveText([
    'Boby McBobFacebob@example.com',
    'Malloryy McMalloryFacemallory@example.com',
