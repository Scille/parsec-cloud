...
msTest.describe(() => {
  ...
  for (const gridMode of [false, true]) {
    msTest(`Move document in ${gridMode ? 'grid' : 'list'} mode`, async ({ documents }, testInfo: TestInfo) => {
      ...
      await expect(opItems.nth(0).locator('.element-details__name')).toHaveText('Move of image.png');
  ...
  for (const dupPolicy of ['replace', 'ignore', 'addCount']) {
    msTest(`Move document with duplicate and ${dupPolicy}`, async ({ documents }) => {
      ...
      const opItems = uploadMenu.locator('.upload-menu-list').locator('.file-operation-item');
      await expect(opItems).toHaveCount(2);
      await expect(opItems.nth(0).locator('.element-details__name')).toHaveText('file.txt');
      await expect(opItems.nth(0).locator('.element-details-info')).toHaveText('5 B •  wksp1');
      await expect(opItems.nth(1).locator('.element-details__name')).toHaveText('file.txt');
      ...
      if (dupPolicy === 'replace') {
        ...
        await expect(opItems.nth(0).locator('.element-details__name')).toHaveText('Move of file.txt');
        await expect(entries).toHaveCount(1);
    ...
    msTest(`Move folder with duplicate and ${dupPolicy}`, async ({ documents }) => {
      ...
      await expect(modal).toBeHidden();
      await expect(dupModal).toBeHidden();
      await DocumentsPage.clickAction(await DocumentsPage.openActionPopover(documents, 0), 'Move to');
      await expect(modal).toBeVisible();
      await expect(modal.locator('.ms-modal-header__title')).toHaveText('Move one item');
      const okButton = modal.locator('#next-button');
      await expect(okButton).toHaveText('Move here');
      ...
      await expect(uploadMenu).toBeVisible();
      await expect(opItems).toHaveCount(3);
      await expect(opItems.nth(0).locator('.element-details__name')).toHaveText('Move of Folder');
