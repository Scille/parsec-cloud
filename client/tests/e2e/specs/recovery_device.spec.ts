...
msTest('Export and use recovery files', async ({ myProfilePage }) => {
  ...
  await expect(recovery.locator('.organization-recovery-container').locator('.danger')).toBeVisible();
  await expect(recovery.locator('.organization-recovery-container').locator('.danger')).toHaveText('Action required');
  ...
  await recovery.locator('.restore-password-button').locator('ion-button').click();
