...
import { expect, fillInputModal, fillIonInput, getOrganizationAddr, login, logout, MsPage, msTest, setupNewPage } from '@tests/e2e/helpers';
...
for (const identitySystem of ['pki', 'openbao']) {
  msTest(`Async enrollment using ${identitySystem}`, async ({ context }) => {
    ...
    if (identitySystem === 'pki') {
      ...
      await expect(requestModal.locator('.user-information').locator('.dropdown-button-content')).toHaveText(
