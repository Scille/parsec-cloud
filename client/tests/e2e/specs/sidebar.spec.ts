...
msTest('Sidebar in connected page', async ({ workspaces }) => {
  ...
  const recentFiles = sidebar.locator('.sidebar-content-files');
  ...
  await expect(recentWorkspaces.locator('.sidebar-content-workspaces--no-recent')).toHaveText('No recent workspaces');
  await expect(recentWorkspaces.locator('.sidebar-content-workspaces__title')).toBeVisible();
  await expect(recentFiles.locator('.sidebar-content-files--no-recent')).toBeVisible();
  await expect(recentFiles.locator('.sidebar-content-files--no-recent')).toHaveText('No recent documents');
  ...
  // await recentWorkspaces.locator('.sidebar-content-workspaces__title')).click();
});
