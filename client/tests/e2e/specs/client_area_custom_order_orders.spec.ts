...
msTest('Test initial status', async ({ clientAreaCustomOrder }) => {
  ...
  await expect(orders.locator('.order-header').locator('.order-header__title')).toHaveText([
    'Order n°YY-00001 Standby',
    'Order n°YY-00002 Available',
  ]);
  const order1 = orders.nth(0).locator('.order-content');
  const order2 = orders.nth(1).locator('.order-content');
  ...
  await expect(order1.locator('.details-list-item').nth(0).locator('.details-list-item__data')).toHaveText([
    '10 administrators',
    '300 members',
    '100 externals',
  ]);
  ...
  await expect(items.nth(1).locator('.details-list-item__data')).toHaveText('120 B');
...
msTest('Initial status with no orders', async ({ clientAreaCustomOrder }) => {
  ...
  await MockBms.mockCustomOrderRequest(clientAreaCustomOrder, undefined, { noRequest: true });
...
msTest('Get custom orders error', async ({ clientAreaCustomOrder }) => {
  ...
  await MockBms.mockCustomOrderRequest(clientAreaCustomOrder, { GET: { timeout: true } });
