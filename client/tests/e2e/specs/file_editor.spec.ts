...
import { expect, login, msTest, skipIfCryptpadUnavailable, waitUntilSaved } from '@tests/e2e/helpers';

msTest(
  'Open editor with header option',
  skipIfCryptpadUnavailable(async ({ parsecEditics }) => {
...
msTest(
  'Open editor with contextual menu',
  skipIfCryptpadUnavailable(async ({ parsecEditics }) => {
    const entries = parsecEditics.locator('.folder-container').locator('.file-list-item');
...
msTest(
  'Edit file in editor with two users',
  skipIfCryptpadUnavailable(async ({ parsecEditics }) => {
    msTest.setTimeout(120_000);
    const entries = parsecEditics.locator('.folder-container').locator('.file-list-item');
...
msTest(
  'Check file edited by other user',
  skipIfCryptpadUnavailable(async ({ parsecEditics }) => {
    msTest.setTimeout(120_000);
    await parsecEditics.locator('.header-label-name').click();
    const entries = parsecEditics.locator('.folder-container').locator('.file-list-item');
