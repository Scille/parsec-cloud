...
For security reasons, the certificate must be provided each time the organization is reopened.`,
  ...
  const invalidRequests = container.locator('.pkiRequests-container-list').locator('.invalid-request');
  ...
  await expect(reqs.nth(0).locator('.pkiRequest-certificate__label')).toHaveText('Valid');
