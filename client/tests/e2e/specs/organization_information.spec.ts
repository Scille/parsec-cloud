...
import { answerQuestion, DEFAULT_USER_INFORMATION, DisplaySize, expect, getClipboardText, mockLibParsec, msTest } from '@tests/e2e/helpers';
...
for (const displaySize of [DisplaySize.Large, DisplaySize.Small]) {
  msTest(`Org info default state on ${displaySize} display with different configuration`, async ({ connected }) => {
    await mockLibParsec(connected, [
      {
        ...
        result: { ok: true, value: { totalMetadataBytes: 2 * 1024 * 1024 * 1024, totalBlockBytes: 16 * 1024 * 1024 * 1024 * 1024 } },
      },
      {
        name: 'clientInfo',
        result: {
          ok: true,
          value: {
            organizationAddr: 'parsec3://addr',
            organizationId: 'BlackMesa',
            deviceId: 'deviceId',
            userId: 'userId',
            deviceLabel: 'Device Label',
            humanHandle: { email: DEFAULT_USER_INFORMATION.email, label: DEFAULT_USER_INFORMATION.name },
            currentProfile: 'UserProfileAdmin',
            serverOrganizationConfig: {
              userProfileOutsiderAllowed: false,
              activeUsersLimit: { tag: 'ActiveUsersLimitLimitedTo', x1: 5 },
              realmMinimumArchivingPeriodBeforeDeletion: 1200,
            },
            isServerOnline: true,
            isOrganizationExpired: false,
            mustAcceptTos: false,
          },
        },
      },
    ...
    await expect(userCategories.nth(2).locator('.users-list-item__title')).toHaveText('0');
    await expect(userCategories.nth(2).locator('.users-list-item__description')).toHaveText('Frozen');
    await expect(usersContainer.locator('.user-active-list').locator('.label-profile')).toHaveText(['Administrator', 'Member']);
    await expect(usersContainer.locator('.user-active-list').locator('.user-active-list-item__value')).toHaveText(['1', '1']);
