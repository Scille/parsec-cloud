...
for (const displaySize of ['small', 'large']) {
  msTest(`Go through saas org creation process with sequester key on ${displaySize} display`, async ({ home }, testInfo: TestInfo) => {
  ...
  msTest(
    `Go through saas org creation process with invalid sequester key on ${displaySize} display`,
  ...
  msTest(
    `Go through saas org creation process from bootstrap link and sequester key on ${displaySize} display`,
