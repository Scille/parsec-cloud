...
for (const displaySize of ['small', 'large']) {
  msTest(`Go through custom org creation process with sequester key on ${displaySize} display`, async ({ home }, testInfo: TestInfo) => {
  ...
  msTest(
    `Go through custom org creation process with invalid sequester key on ${displaySize} display`,
    async ({ home }, testInfo: TestInfo) => {
      ...
      await expect(summaryContainer.locator('.form-error')).toHaveText('The provided sequester key is not valid.');
    },
  );

  msTest(
    `Go through custom org creation process from bootstrap link with sequester key on ${displaySize} display`,
