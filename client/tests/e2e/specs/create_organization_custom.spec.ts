...
msTest('Go through custom org creation process with sequester key', async ({ home }, testInfo: TestInfo) => {
  const modal = await openCreateOrganizationModal(home);
  ...
  await expect(orgServerContainer.locator('.sequester-container').locator('.upload-key__button')).toHaveText('Add verification key');
...
msTest('Go through custom org creation process with invalid sequester key', async ({ home }, testInfo: TestInfo) => {
  const modal = await openCreateOrganizationModal(home);
  ...
  await expect(orgServerContainer.locator('.sequester-container').locator('.upload-key-update').locator('ion-text')).toHaveText('text.txt');
...
msTest('Go through custom org creation process from bootstrap link with sequester key', async ({ context }, testInfo: TestInfo) => {
  const page = (await context.newPage()) as MsPage;
  // Making sure that the testbed is recognized as the custom server
  await setupNewPage(page, { trialServers: 'unknown.host', saasServers: 'unknown.host' });
  ...
  // Name, email, sequester key and authentication fields can be updated, server & org name cannot
  await expect(summaryEditButtons.nth(0)).not.toBeVisible();
  await expect(summaryEditButtons.nth(1)).toBeVisible();
  await expect(summaryEditButtons.nth(2)).toBeVisible();
  await expect(summaryEditButtons.nth(3)).toBeVisible();
