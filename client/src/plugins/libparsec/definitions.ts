...
export type CancelError = CancelErrorInternal | CancelErrorNotBound;
...
export type ClaimerGreeterAbortOperationError = ClaimerGreeterAbortOperationErrorInternal;
...
export type ClientCreateWorkspaceError = ClientCreateWorkspaceErrorInternal;
...
export type ClientEvent = ClientEventPing;
...
export type ClientGetUserDeviceError = ClientGetUserDeviceErrorInternal | ClientGetUserDeviceErrorNonExisting;
...
export type ClientInfoError = ClientInfoErrorInternal;
...
export type ClientListUserDevicesError = ClientListUserDevicesErrorInternal;
...
export type ClientListUsersError = ClientListUsersErrorInternal;
...
export type ClientListWorkspaceUsersError = ClientListWorkspaceUsersErrorInternal;
...
export type ClientListWorkspacesError = ClientListWorkspacesErrorInternal;
...
export type ClientRenameWorkspaceError = ClientRenameWorkspaceErrorInternal | ClientRenameWorkspaceErrorUnknownWorkspace;
...
export type ClientStartInvitationGreetError = ClientStartInvitationGreetErrorInternal;
...
export type ClientStartWorkspaceError = ClientStartWorkspaceErrorInternal | ClientStartWorkspaceErrorNoAccess;
...
export type ClientStopError = ClientStopErrorInternal;
...
export type DeviceAccessStrategy = DeviceAccessStrategyPassword | DeviceAccessStrategySmartcard;
...
export type DeviceSaveStrategy = DeviceSaveStrategyPassword | DeviceSaveStrategySmartcard;
...
export type EntryStat = EntryStatFile | EntryStatFolder;
...
export type InviteListItem = InviteListItemDevice | InviteListItemUser;
...
export type ListInvitationsError = ListInvitationsErrorInternal | ListInvitationsErrorOffline;
...
export type ParseBackendAddrError = ParseBackendAddrErrorInvalidUrl;
...
export type UserOrDeviceClaimInitialInfo = UserOrDeviceClaimInitialInfoDevice | UserOrDeviceClaimInitialInfoUser;
...
export type WorkspaceStopError = WorkspaceStopErrorInternal;
...
export type WorkspaceStorageCacheSize = WorkspaceStorageCacheSizeCustom | WorkspaceStorageCacheSizeDefault;
...
export interface LibParsecPlugin {
  ...
  buildBackendOrganizationBootstrapAddr(addr: BackendAddr, organization_id: OrganizationID): Promise<BackendOrganizationBootstrapAddr>;
  cancel(canceller: Handle): Promise<Result<null, CancelError>>;
  ...
  claimerDeviceInitialDoWaitPeer(canceller: Handle, handle: Handle): Promise<Result<DeviceClaimInProgress1Info, ClaimInProgressError>>;
  claimerGreeterAbortOperation(handle: Handle): Promise<Result<null, ClaimerGreeterAbortOperationError>>;
  claimerRetrieveInfo(
    config: ClientConfig,
    on_event_callback: (event: ClientEvent) => void,
    addr: BackendInvitationAddr,
  ...
  claimerUserInProgress1DoSignifyTrust(canceller: Handle, handle: Handle): Promise<Result<UserClaimInProgress2Info, ClaimInProgressError>>;
  claimerUserInProgress2DoWaitPeerTrust(canceller: Handle, handle: Handle): Promise<Result<UserClaimInProgress3Info, ClaimInProgressError>>;
  ...
  claimerUserInitialDoWaitPeer(canceller: Handle, handle: Handle): Promise<Result<UserClaimInProgress1Info, ClaimInProgressError>>;
  clientCreateWorkspace(client: Handle, name: EntryName): Promise<Result<VlobID, ClientCreateWorkspaceError>>;
  clientDeleteInvitation(client: Handle, token: InvitationToken): Promise<Result<null, DeleteInvitationError>>;
  clientGetUserDevice(client: Handle, device: DeviceID): Promise<Result<[UserInfo, DeviceInfo], ClientGetUserDeviceError>>;
  clientInfo(client: Handle): Promise<Result<ClientInfo, ClientInfoError>>;
  clientListInvitations(client: Handle): Promise<Result<Array<InviteListItem>, ListInvitationsError>>;
  clientListUserDevices(client: Handle, user: UserID): Promise<Result<Array<DeviceInfo>, ClientListUserDevicesError>>;
  clientListUsers(client: Handle, skip_revoked: boolean): Promise<Result<Array<UserInfo>, ClientListUsersError>>;
  clientListWorkspaceUsers(
    client: Handle,
    realm_id: VlobID,
  ): Promise<Result<Array<WorkspaceUserAccessInfo>, ClientListWorkspaceUsersError>>;
  clientListWorkspaces(client: Handle): Promise<Result<Array<WorkspaceInfo>, ClientListWorkspacesError>>;
  clientNewDeviceInvitation(client: Handle, send_email: boolean): Promise<Result<NewInvitationInfo, NewDeviceInvitationError>>;
  clientNewUserInvitation(
    client: Handle,
    claimer_email: string,
    send_email: boolean,
  ): Promise<Result<NewInvitationInfo, NewUserInvitationError>>;
  clientRenameWorkspace(client: Handle, realm_id: VlobID, new_name: EntryName): Promise<Result<null, ClientRenameWorkspaceError>>;
  ...
  clientStartWorkspace(client: Handle, realm_id: VlobID): Promise<Result<Handle, ClientStartWorkspaceError>>;
  clientStop(client: Handle): Promise<Result<null, ClientStopError>>;
  getDefaultConfigDir(): Promise<Path>;
  getDefaultDataBaseDir(): Promise<Path>;
  getDefaultMountpointBaseDir(): Promise<Path>;
  ...
  greeterDeviceInitialDoWaitPeer(canceller: Handle, handle: Handle): Promise<Result<DeviceGreetInProgress1Info, GreetInProgressError>>;
  greeterUserInProgress1DoWaitPeerTrust(canceller: Handle, handle: Handle): Promise<Result<UserGreetInProgress2Info, GreetInProgressError>>;
  greeterUserInProgress2DoSignifyTrust(canceller: Handle, handle: Handle): Promise<Result<UserGreetInProgress3Info, GreetInProgressError>>;
  ...
  greeterUserInitialDoWaitPeer(canceller: Handle, handle: Handle): Promise<Result<UserGreetInProgress1Info, GreetInProgressError>>;
  listAvailableDevices(path: Path): Promise<Array<AvailableDevice>>;
  newCanceller(): Promise<Handle>;
  parseBackendAddr(url: string): Promise<Result<ParsedBackendAddr, ParseBackendAddrError>>;
  testDropTestbed(path: Path): Promise<null>;
  testGetTestbedBootstrapOrganizationAddr(discriminant_dir: Path): Promise<BackendOrganizationBootstrapAddr | null>;
  testGetTestbedOrganizationId(discriminant_dir: Path): Promise<OrganizationID | null>;
  testNewTestbed(template: string, test_server: BackendAddr | null): Promise<Path>;
  validateDeviceLabel(raw: string): Promise<boolean>;
  validateEmail(raw: string): Promise<boolean>;
  validateEntryName(raw: string): Promise<boolean>;
  validateHumanHandleLabel(raw: string): Promise<boolean>;
  validateInvitationToken(raw: string): Promise<boolean>;
  validateOrganizationId(raw: string): Promise<boolean>;
  validatePath(raw: string): Promise<boolean>;
  workspaceCreateFile(workspace: Handle, path: FsPath): Promise<Result<VlobID, WorkspaceFsOperationError>>;
  workspaceCreateFolder(workspace: Handle, path: FsPath): Promise<Result<VlobID, WorkspaceFsOperationError>>;
  workspaceCreateFolderAll(workspace: Handle, path: FsPath): Promise<Result<VlobID, WorkspaceFsOperationError>>;
  workspaceRemoveEntry(workspace: Handle, path: FsPath): Promise<Result<null, WorkspaceFsOperationError>>;
  workspaceRemoveFile(workspace: Handle, path: FsPath): Promise<Result<null, WorkspaceFsOperationError>>;
  workspaceRemoveFolder(workspace: Handle, path: FsPath): Promise<Result<null, WorkspaceFsOperationError>>;
  workspaceRemoveFolderAll(workspace: Handle, path: FsPath): Promise<Result<null, WorkspaceFsOperationError>>;
  ...
  workspaceStatEntry(workspace: Handle, path: FsPath): Promise<Result<EntryStat, WorkspaceFsOperationError>>;
  workspaceStop(workspace: Handle): Promise<Result<null, WorkspaceStopError>>;
}
