...
self.onconnect = async (msg: MessageEvent): Promise<void> => {
  ...
  // This need to be done after we setup the event listeners.
  console.log('Worker is ready to process event from client');
  port.start();
};

let _libparsec: LibParsecPlugin | undefined = undefined;
async function maybeInit(): Promise<LibParsecPlugin> {
  // Initialization is done once when the shared worker is created.
