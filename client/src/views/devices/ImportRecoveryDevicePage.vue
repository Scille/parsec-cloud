...
import { AvailableDevice, DeviceInfo, importRecoveryDevice, ImportRecoveryDeviceErrorTag } from '@/parsec';
...
async function createNewDevice(): Promise<void> {
  ...
  let content = new Uint8Array(recoveryFile.value.size);
  let offset = 0;
  ...
  console.log(content.length, content.at(0), content.at(content.length - 1));

  const result = await importRecoveryDevice(props.device.deviceLabel, content, secretKey.value, chooseAuthRef.value.getSaveStrategy());
  console.log(result);
  if (result.ok) {
