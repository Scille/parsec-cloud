...
async function downloadRecoveryFile(): Promise<void> {
  await downloadFile(content, 'application/octet-stream', { key: 'ExportRecoveryDevicePage.filenames.recoveryFile', data: { org: orgId.value } });
  setTimeout(() => {
...
async function downloadFile(data: Uint8Array | string, contentType: 'application/octet-stream' | 'text/plain', fileName: Translatable): Promise<void> {
