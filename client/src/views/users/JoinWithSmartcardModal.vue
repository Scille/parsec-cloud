...
async function onJoinClicked(): Promise<boolean> {
  if (!areInfoValid.value || !userInformationRef.value || !certificate.value) {
    return false;
  }

  const result = await requestJoinOrganization(
    toRaw(certificate.value),
    { label: userInformationRef.value.getFullName(), email: userInformationRef.value.getEmail() },
    props.addr,
  );
  ...
  if (result.error.tag === PkiEnrollmentSubmitErrorTag.EmailAlreadyUsed) {
    error.value = 'HomePage.organizationRequest.requestSent.emailUsed';
  } else if (result.error.tag === PkiEnrollmentSubmitErrorTag.AlreadyEnrolled) {
    error.value = 'HomePage.organizationRequest.requestSent.alreadyEnrolled';
  } else if (result.error.tag === PkiEnrollmentSubmitErrorTag.AlreadySubmitted) {
    error.value = 'HomePage.organizationRequest.requestSent.alreadySubmitted';
  } else if (result.error.tag === PkiEnrollmentSubmitErrorTag.Offline) {
    error.value = 'HomePage.organizationRequest.requestSent.offline';
  } else {
    error.value = 'HomePage.organizationRequest.requestSent.failure';
  }
  return false;
}
