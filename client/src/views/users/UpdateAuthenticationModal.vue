...
import { MsModalResult, Translatable, asyncComputed, useWindowSize } from 'megashark-lib';
...
async function changeAuthentication(): Promise<void> {
  ...
  const result = await updateDeviceChangeAuthentication(constructAccessStrategy(props.currentDevice, toRaw(currentAuth.value)), saveStrategy);
