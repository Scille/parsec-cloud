        ...
        <ion-list class="error-advices-list ion-no-padding">
          ...
          <ion-item class="error-advices-list__item ion-no-padding body">
            <ion-icon
              ...
              :icon="checkmarkCircle"
            />
            {{ $msTranslate('fileEditors.advices.advice4') }}
          </ion-item>
        </ion-list>
      </div>
    </div>
...
async function openFileWithCryptpad(): Promise<void> {
  ...
  const fileData = new Uint8Array(contentInfo.data);

  fileUrl.value = URL.createObjectURL(new Blob([fileData], { type: 'application/octet-stream' }));
