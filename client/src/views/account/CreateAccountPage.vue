...
async function createAccount(): Promise<void> {
  ...
  try {
    ...
    const result = await creationStepper.createAccount(ParsecAccountAccess.usePassword(choosePasswordRef.value?.password));
    ...
    if (!result.ok) {
      if (result.error.tag === AccountCreateProceedErrorTag.Offline) {
