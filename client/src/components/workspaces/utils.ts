        ...
        if (storageManager !== undefined) {
          if (isDesktop()) {
            ...
            if (refreshWorkspaces.ok) {
              await unmountWorkspaceConfirmation(workspaceAttributes,
                refreshWorkspaces.value, informationManager, eventDistributor, storageManager);
            }
          } else {
...
export async function hideWorkspace(
  ...
  let ok = true;
  if (isDesktop()) {
    const result = await unmountWorkspace(workspace as WorkspaceInfo);
    ok = result.ok;
  }
