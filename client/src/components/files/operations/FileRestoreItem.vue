...
<template>
  <div
    ...
    :class="{
      waiting: state === FileOperationState.RestoreAdded,
      progress: state === FileOperationState.OperationProgress,
      done: state === FileOperationState.EntryRestored,
      cancelled: state === FileOperationState.Cancelled,
      failed: state === FileOperationState.RestoreFailed,
    }"
  >
    <ion-item class="element ion-no-padding">
      ...
      <div class="element-details">
        <ion-text class="element-details__name body">
          {{
            $msTranslate({
              key: 'FoldersPage.RestoreFile.restore',
        ...
        >
          ...
          >
            <ion-icon
              class="cancel-icon"
              v-if="state === FileOperationState.RestoreAdded"
      ...
      >
        <ion-text class="waiting-text body">
          {{ $msTranslate('FoldersPage.RestoreFile.waiting') }}
        </ion-text>
        <ion-icon
          class="cancel-icon"
          @click="$emit('cancel', operationData.id)"
      ...
      <ion-icon
        ...
        v-show="state === FileOperationState.EntryRestored"
      ...
      >
        {{ $msTranslate('FoldersPage.RestoreFile.cancelled') }}
      </ion-text>

      <!-- failed -->
      <div
        class="failed-content"
        v-if="state === FileOperationState.RestoreFailed"
      >
        <ion-text class="failed-text body">
          {{ $msTranslate('FoldersPage.RestoreFile.failed') }}
        </ion-text>
        <ms-information-tooltip
          v-show="false"
          :text="'FoldersPage.RestoreFile.failedDetails'"
...
import { FileOperationState, OperationProgressStateData, RestoreData, StateData } from '@/services/fileOperation/manager';
import { IonButton, IonIcon, IonItem, IonLabel, IonText } from '@ionic/vue';
import { checkmarkCircle, closeCircle } from 'ionicons/icons';
import { MsImage, MsInformationTooltip, MsProgress, MsProgressAppearance, Restore } from 'megashark-lib';
import { Ref, onMounted, ref } from 'vue';

const props = defineProps<{
  operationData: RestoreData;
  stateData?: StateData;
...
onMounted(async () => {
  ...
  entryName.value = (await Path.filename(props.operationData.path)) ?? '';
});

defineEmits<{
  (event: 'cancel', id: string): void;
  (event: 'click', operationData: RestoreData, state: FileOperationState): void;
}>();

function getProgress(): number {
  if (props.state === FileOperationState.Cancelled || props.state === FileOperationState.EntryRestored) {
    return 100;
