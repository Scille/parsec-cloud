...
<template>
  <div class="search-results">
    <div
      v-if="props.minCharsNeeded"
      class="search-results__empty"
    >
      <ion-text class="body">{{ $msTranslate('workspaceHistory.searchMinChars') }}</ion-text>
    </div>
    <div
    ...
    >
      ...
      <ion-text class="body">{{ $msTranslate('workspaceHistory.search.searching') }}</ion-text>
    </div>
    <div
      v-else-if="!props.isSearching && props.results.length === 0"
      class="search-results__empty"
    >
      <ion-text class="body">{{ $msTranslate('workspaceHistory.search.noResults') }}</ion-text>
    </div>
    <div
      v-else
...
import { WorkspaceHistorySearchMatch } from '@/parsec/types';
import { WorkspaceHistoryEntryStatTag } from '@/plugins/libparsec';
import { IonText } from '@ionic/vue';
import { Folder, MsImage, MsSpinner } from 'megashark-lib';

const props = defineProps<{
  results: WorkspaceHistorySearchMatch[];
  isSearching: boolean;
  minCharsNeeded?: boolean;
}>();

defineEmits<{
  (e: 'resultClick', match: WorkspaceHistorySearchMatch): void;
}>();
...
function isFile(match: WorkspaceHistorySearchMatch): boolean {
  return match.stat.tag === WorkspaceHistoryEntryStatTag.File;
}
...
function getHighlightedParts(match: WorkspaceHistorySearchMatch): PathPart[] {
