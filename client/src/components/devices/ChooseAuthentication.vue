  ...
  >
    ...
    >
      ...
      >
        ...
        <ion-radio
          v-show="serverConfig?.openbao && serverConfig?.openbao.auths.some((auth) => isSSOProviderHandled(auth.tag))"
          class="item-radio radio-list-item"
          label-placement="end"
    ...
    >
      ...
      <div
        v-if="
          authentication === DeviceSaveStrategyTag.OpenBao &&
          serverConfig?.openbao &&
          serverConfig?.openbao.auths.some((auth) => isSSOProviderHandled(auth.tag))
        "
      >
