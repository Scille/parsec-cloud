...
async function handlePkiLink(link: string): Promise<void> {
  if (!currentRouteIs(Routes.Home)) {
    await switchOrganization(null, true);
  }
  await navigateTo(Routes.Home, { query: { pkiLink: link } });
}
...
export async function handleParsecLink(link: string, informationManager: InformationManager): Promise<void> {
  ...
  } else if (result.value.tag === ParsedParsecAddrTag.PkiEnrollment) {
    await handlePkiLink(link);
  }
}
