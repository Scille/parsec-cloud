...
export default function useFileOpener(): PathOpener {
  ...
  async function _openWithSystem(workspaceHandle: WorkspaceHandle, entry: EntryStat | WorkspaceHistoryEntryStat): Promise<void> {
    if (entry.isFile()) {
  ...
  async function showInExplorer(workspaceHandle: WorkspaceHandle, path: FsPath): Promise<void> {
  ...
  async function openPath(workspaceHandle: WorkspaceHandle, path: FsPath, options: OpenPathOptions): Promise<void> {
    ...
    if (Env.isEditicsEnabled() && isOnlyOfficeEnabledForDocumentType(contentType.type)) {
