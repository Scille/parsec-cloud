...
async function openBaoConnect(
  ...
  try {
    ...
    client = new OpenBaoClient(result.value.token, result.value.id, openBaoServer, provider, mountpoint, secretMountpoint);
...
export { getOpenBaoClient, isOpenBaoAvailable, isSSOProviderHandled, openBaoConnect, OpenBaoError, OpenBaoResult };
