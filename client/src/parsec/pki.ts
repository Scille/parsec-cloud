...
const _PKI_PARSEC_API = {
  async requestJoinOrganization(
    ...
    const result = await libparsec.pkiEnrollmentSubmit(getClientConfig(), link, certificate, humanHandle, getDefaultDeviceName(), true);
    if (result.ok) {
  ...
  async cancelLocalJoinRequest(_request: PkiLocalRequest): Promise<Result<null, null>> {
    return { ok: true, value: null };
  },
    ...
    const REQUESTS: Array<PkiLocalRequest> = [
      {
        ...
        enrollment: {
          ...
          addr: 'parsec3://localhost:6770/MyOrg?no_ssl=true&a=pki_enrollment',
          submittedOn: DateTime.utc(),
          enrollmentId: '1',
          payload: PAYLOAD,
