...
import {
  ...
  SubmitterForgetAsyncEnrollmentErrorTag,
...
const _ASYNC_ENROLLMENT_MOCKED_API = {
  ...
  async deleteJoinRequest(request: AsyncEnrollmentRequest): Promise<Result<null, SubmitterForgetAsyncEnrollmentError>> {
    ...
    if (reqIndex === -1) {
      return { ok: false, error: { tag: SubmitterForgetAsyncEnrollmentErrorTag.NotFound, error: 'not found' } };
    }
    REQUESTS.splice(reqIndex, 1);
    return { ok: true, value: null };
  },
  ...
  async listAsyncEnrollments(): Promise<Result<Array<AsyncEnrollmentUntrusted>, ClientListAsyncEnrollmentsError>> {
    ...
    const ENROLLMENTS: Array<AsyncEnrollmentUntrusted> = REQUESTS.map((req) => {
