...
export class WorkspaceHistory {
  ...
  async start(currentTime?: DateTime): Promise<Result<WorkspaceHistoryHandle, ClientStartWorkspaceHistory2Error>> {
    ...
    if (connectionHandle && !needsMocks()) {
      ...
      if (MOCK_OPENED_HISTORY.values().find((v) => v === this.workspaceId) !== undefined) {
  ...
  async setCurrentTime(time: DateTime): Promise<Result<DateTime, WorkspaceHistory2InternalOnlyError>> {
    if (this.handle === undefined) {
      return { ok: false, error: { tag: WorkspaceHistory2InternalOnlyErrorTag.Internal, error: 'Not started' } };
    }
    if (!needsMocks()) {
