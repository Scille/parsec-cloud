...
const workerPluginsFactories: (() => PluginOption)[] = [() => vue(), () => topLevelAwait()];
let platform: string;
...
if (platform === 'web') {
  ...
  workerPluginsFactories.push(() => wasmPack([{ path: '../bindings/web/', name: 'libparsec_bindings_web' }], false));
}

if (process.env.PARSEC_APP_SENTRY_AUTH_TOKEN) {
  const sentryPluginFactory = () =>
    sentryVitePlugin({
      org: 'scille',
