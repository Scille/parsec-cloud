...
async fn ok(env: &TestbedEnv, mocked: bool) {
    let account_token = AccountToken::default(); // TODO add account token to testbed
                                                 //  = env
    ...
    let addr = ParsecAuthenticatedAccountAddr::new(env.server_addr.clone(), account_token);
    let cmds =
...
async fn invalid_token(env: &TestbedEnv, mocked: bool) {
    ...
    let bad_addr =
        ParsecAuthenticatedAccountAddr::new(env.server_addr.clone(), AccountToken::default());
    let cmds =
