...
macro_rules! test_send_hook_vault_item_list {
    ...
    ($env: expr, $auth_method_secret_key: expr, $vault_key: expr, web_local_device_keys: [$(($web_local_device: expr, $web_local_device_key: expr)),* $(,)?] $(,)?) => {{
        let env: &TestbedEnv = $env;
        let auth_method_secret_key: &SecretKey = &$auth_method_secret_key;
        let vault_key: &SecretKey = &$vault_key;
        ...
        let mut items = std::collections::HashMap::new();
        $(
            let web_local_device: &LocalDevice = &$web_local_device;
            let encrypted_data = WebLocalDeviceKeyAccess {
                web_local_device_key: $web_local_device_key.to_owned(),
            }.dump_and_encrypt(&vault_key).into();
            let item = AccountVaultItemWebLocalDeviceKey {
                organization_id: web_local_device.organization_id().to_owned(),
                device_id: web_local_device.device_id,
                encrypted_data,
            };
            items.insert(item.fingerprint(), item.dump().into());
        )*
