...
pub enum ConnectionError {
    ...
    /// This should never happen since the server always include that header
    /// when API version sent by the client is not supported
    ...
    /// User has beed revoked
    #[error("User has been revoked")]
    RevokedUser,

    /// User has beed frozen (temporarily suspended from the server)
