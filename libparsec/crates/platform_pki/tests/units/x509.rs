...
use hex_literal::hex;
use libparsec_types::HumanHandleParseError;
...
#[test]
fn missing_name_for_human_handle() {
    let info = X509CertificateInformation {
        subject: vec![DistinguishedNameValue::EmailAddress(
...
#[test]
fn missing_email_for_human_handle() {
    let info = X509CertificateInformation {
        subject: vec![DistinguishedNameValue::CommonName("foo".into())],
        issuer: Default::default(),
        extensions: Default::default(),
    };
    assert!(matches!(
...
#[test]
fn invalid_name_for_human_handle() {
    let info = X509CertificateInformation {
        subject: vec![
            DistinguishedNameValue::CommonName("<foo>".into()),
            DistinguishedNameValue::EmailAddress("foo@example.com".into()),
        ],
        issuer: Default::default(),
        extensions: Default::default(),
    };
...
#[test]
fn invalid_email_for_human_handle() {
    let info = X509CertificateInformation {
        subject: vec![
            DistinguishedNameValue::CommonName("foo".into()),
            DistinguishedNameValue::EmailAddress("not_an_email".into()),
        ],
        issuer: Default::default(),
        extensions: Default::default(),
    };
