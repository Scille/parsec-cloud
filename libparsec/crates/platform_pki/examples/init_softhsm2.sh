...
for user in "${users[@]}"; do
    ...
    pkcs11-tool --module $TEST_PKCS11_MODULE --login --pin "$SLOT_USER_PIN" --slot "$SOFTHSM2_SLOT" --write-object "$WORKDIR/$user-crt.der" --type cert --id "${id}" --label "$user certificate"
    echo "Importing $user privkey into pkcs11 module"
    pkcs11-tool --module $TEST_PKCS11_MODULE --login --pin "$SLOT_USER_PIN" --slot "$SOFTHSM2_SLOT" --write-object "$WORKDIR/$user-key.der" --type privkey --id "${id}" --label "$user private key"
    echo "Importing $user pubkey into pkcs11 module"
    pkcs11-tool --module $TEST_PKCS11_MODULE --login --pin "$SLOT_USER_PIN" --slot "$SOFTHSM2_SLOT" --write-object "$WORKDIR/$user.pub" --type pubkey --id "${id}" --label "$user public key"
done
