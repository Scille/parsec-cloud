...
impl Directory {
    pub async fn get_root_dir() -> Result<Self, GetRootDirectoryError> {
    ...
    pub async fn get_dir_handle_from_path(
        &self,
        path: &Path,
        ...
        let dir = self
            .get_parent_dir_handle(path.parent(), option.clone())
            .await?;
        dir.as_ref()
            .unwrap_or(self)
            .get_dir_handle(dirname, option)
            .await
    }

    async fn get_parent_dir_handle(
        &self,
        parent: Option<&Path>,
        option: Option<OpenOptions>,
        ...
        if let Some(parent) = parent {
            let dir = self
                .get_dir_handle_from_path(parent, option.clone())
                .boxed_local()
                .await?;
            Ok(Some(dir))
        } else {
    ...
    pub async fn create_dir_all(&self, path: &Path) -> Result<Self, GetDirectoryHandleError> {
        self.get_dir_handle_from_path(path, Some(OpenOptions::create()))
            .await
    }
    ...
    pub async fn get_file_handle_from_path(
        &self,
        path: &Path,
        option: Option<OpenOptions>,
        ...
        let dir = self.get_parent_dir_handle(path.parent(), None).await?;
        dir.as_ref()
            .unwrap_or(self)
            .get_file_handle(filename, option)
            .await
    }
