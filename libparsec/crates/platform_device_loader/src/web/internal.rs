...
impl Storage {
    pub(crate) async fn new() -> Result<Self, NewStorageError> {
        let root_dir = Directory::get_root_dir().await?;
        Ok(Self { root_dir })
    }
    ...
    pub(crate) async fn list_available_devices(
        ...
        let dir = match self
            .root_dir
            .get_dir_handle_from_path(&devices_dir, None)
            .await
        {
            Ok(dir) => dir,
        ...
        while let Some(dir) = {
            ...
            let available_device_stream = dir.entries().filter_map(|entry| async {
                ...
                match handle {
                        ...
                        if path.extension() == Some(crate::DEVICE_FILE_EXT.as_ref()) =>
                    {
                        ...
                        Some(
                            load_available_device(&File { path, handle })
                                .await
                                .map_err(ListAvailableDevicesError::from),
                        )
                    }
            ...
            devices.extend(available_device_stream.try_collect::<Vec<_>>().await?);
        }
    ...
    pub(crate) async fn load_device(
        ...
        let file = self
            .root_dir
            .get_file_handle_from_path(access.key_file(), None)
            .await?;
    ...
    pub(crate) async fn archive_device(&self, path: &Path) -> Result<(), ArchiveDeviceError> {
        let old_device = self
            .root_dir
            .get_file_handle_from_path(path, None)
            .await
        ...
        let archive_device = self
            .root_dir
            .get_file_handle_from_path(&archive_path, Some(OpenOptions::create()))
            .await
...
async fn load_available_device(file: &File) -> Result<AvailableDevice, LoadAvailableDeviceError> {
    ...
    crate::load_available_device_from_blob(file.path().to_owned(), &raw_data)
        .inspect_err(|e| log::warn!("Failed to load device from {}: {e}", file.path().display()))
        .map_err(Into::into)
}
