...
pub async fn list_pending_async_enrollments(
    ...
    platform::list_pending_async_enrollments(config_dir).await
}

fn load_pending_async_enrollment(
    path: PathBuf,
    raw: &[u8],
    ...
    let content = AsyncEnrollmentLocalPendingCleartextContent::load(&cooked.cleartext_content)
        .inspect_err(
