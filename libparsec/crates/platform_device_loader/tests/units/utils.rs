...
mod platform {
    ...
    async fn get_storage() -> Directory {
        Directory::get_root_dir().await.expect("Cannot get storage")
    }
    ...
    pub async fn create_device_file(path: &Path, content: &[u8]) {
        ...
        let file = dir
            .as_ref()
            .unwrap_or(&storage)
            .get_file_handle(filename, Some(OpenOptions::create()))
            .await
    ...
    pub async fn key_present_in_system(path: &Path) -> bool {
        let storage = get_storage().await;
        match storage.get_file_handle_from_path(path, None).await {
            Ok(_) => true,
