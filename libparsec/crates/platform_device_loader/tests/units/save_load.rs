    ...
    let (access, expected_available_device) = match kind {
        ...
        "smartcard" => {
            let access = DeviceAccessStrategy::Smartcard {
                key_file: key_file.clone(),
                certificate_reference: CertificateReference::IdOrHash {
                    id_or_hash: CertificateReferenceIdOrHash {
                        id: Bytes::copy_from_slice(b"smartcard certificate"),
                        hash: CertificateHash::SHA256 {
                            data: Box::from([0; 32]), // TODO configure smartcard certificate in #11269
                        },
                    },
                },
            };
