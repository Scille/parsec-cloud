...
pub enum TOTPSetupStatus {
    /// The TOTP setup is not yet confirmed: the secret should be displayed
    /// to the end-user so that he can configure his authenticator app.
    Stalled {
...
pub async fn totp_setup_status_authenticated(
    ...
    match rep {
        authenticated_cmds::latest::totp_setup_get_secret::Rep::Ok { totp_secret } => {
            ...
            Ok(TOTPSetupStatus::Stalled { base32_totp_secret })
        }
        authenticated_cmds::latest::totp_setup_get_secret::Rep::AlreadySetup => {
            Ok(TOTPSetupStatus::AlreadySetup)
        }
