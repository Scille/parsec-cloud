...
impl PlatformWorkspaceStorage {
        ...
        with_transaction(
            ...
            async |transaction: Transaction<CustomErrMarker>| {
                let store = transaction
                    .object_store(CHECKPOINT_STORE)
                    .map_err(anyhow::Error::from)?;

                let maybe_singleton = store
                    .get(&CHECKPOINT_SINGLETON_KEY.into())
                    .await
                    .map_err(anyhow::Error::from)?;
    ...
    pub async fn list_manifests(
        ...
        with_transaction(
            ...
            async |transaction: Transaction<CustomErrMarker>| {
                ...
                let mut cursor = store.cursor().open().await.map_err(anyhow::Error::from)?;

                if offset > 0 {
                    cursor.advance(offset).await.map_err(anyhow::Error::from)?;
                }
    ...
    pub async fn get_chunk(
        ...
        with_transaction(
            ...
            async |transaction: Transaction<CustomErrMarker>| {
                ...
                let chunk_id_js = js_sys::Uint8Array::from(chunk_id.as_bytes());
                let maybe_obj = store.get(&chunk_id_js).await.map_err(anyhow::Error::from)?;
    ...
    async fn may_cleanup_blocks(&self, store: &ObjectStore<CustomErrMarker>) -> anyhow::Result<()> {
        let nb_blocks = store.count().await.map_err(anyhow::Error::from)? as u64;
        ...
        if extra_blocks > 0 {
            ...
            for _ in 0..to_remove {
                ...
                cursor.delete().await.map_err(anyhow::Error::from)?;
            }
        }
        ...
        with_transaction(
            ...
            async |transaction: Transaction<CustomErrMarker>| {
                ...
                let pattern = {
                    ...
                    PreventSyncPattern::from_regex(&raw).map_err(anyhow::Error::from)?
                };
