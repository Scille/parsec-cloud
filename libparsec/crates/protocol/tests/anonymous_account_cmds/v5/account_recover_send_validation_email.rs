...
pub fn req() {
    ...
    let req = anonymous_account_cmds::account_recover_send_validation_email::Req {
        email: "alice@invalid.com".parse().unwrap(),
    };

    let expected =
        anonymous_account_cmds::AnyCmdReq::AccountRecoverSendValidationEmail(req.clone());
    ...
    let anonymous_account_cmds::AnyCmdReq::AccountRecoverSendValidationEmail(req2) = data else {
        unreachable!()
    };
...
pub fn rep_ok() {
    ...
    let expected = anonymous_account_cmds::account_recover_send_validation_email::Rep::Ok {};
...
pub fn rep_email_server_unavailable() {
    ...
    let expected =
        anonymous_account_cmds::account_recover_send_validation_email::Rep::EmailServerUnavailable {};
    let data =
        anonymous_account_cmds::account_recover_send_validation_email::Rep::load(raw).unwrap();
...
pub fn rep_email_recipient_refused() {
    ...
    let expected =
        anonymous_account_cmds::account_recover_send_validation_email::Rep::EmailRecipientRefused {};
    let data =
        anonymous_account_cmds::account_recover_send_validation_email::Rep::load(raw).unwrap();
    println!("***expected: {:?}", expected.dump().unwrap());
    p_assert_eq!(data, expected);
