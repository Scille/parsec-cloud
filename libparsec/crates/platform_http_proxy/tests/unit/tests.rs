...
fn bad_http_proxy() {
    ...
    assert_eq!(format!("{:?}", err), "Invalid HTTP proxy configuration: builder error: builder error: relative URL without a base");
}
...
fn bad_https_proxy() {
    ...
    let err = outcome.as_ref().unwrap_err();
    assert_eq!(format!("{:?}", err), "Invalid HTTPS proxy configuration: builder error: builder error: relative URL without a base");
}
