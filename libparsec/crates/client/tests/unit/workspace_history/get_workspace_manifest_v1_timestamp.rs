...
use crate::workspace_history::WorkspaceHistoryGetWorkspaceManifestV1TimestampError;
...
async fn v1_exists(env: &TestbedEnv) {
    ...
    p_assert_eq!(
        ops.get_workspace_manifest_v1_timestamp().await.unwrap(),
...
async fn offline(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.get_workspace_manifest_v1_timestamp().await.unwrap_err(),
...
async fn no_realm_access(env: &TestbedEnv) {
    ...
    let ops = workspace_history_ops_with_server_access_factory(env, &mallory, wksp1_id).await;
