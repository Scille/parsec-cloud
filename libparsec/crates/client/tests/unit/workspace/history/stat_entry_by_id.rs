...
async fn ok_folder(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history.stat_entry_by_id(wksp1_foo_v2_children_available_timestamp, wksp1_id).await,
...
async fn ok_file(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history.stat_entry_by_id(wksp1_foo_v2_children_available_timestamp, wksp1_bar_txt_id).await,
...
async fn before_realm_exists(env: &TestbedEnv) {
    ...
    test_register_sequence_of_send_hooks!(
        &env.discriminant_dir,
        // 1) Get back the workspace manifest, but at this time
    ...
    p_assert_matches!(
        ops.history.stat_entry_by_id(at_before, wksp1_id).await,
...
async fn before_workspace_manifest_v1(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history
            .stat_entry_by_id(wksp1_bootstrapped_timestamp, wksp1_id)
            .await,
        Err(WorkspaceHistoryStatEntryError::EntryNotFound)
    );
}
