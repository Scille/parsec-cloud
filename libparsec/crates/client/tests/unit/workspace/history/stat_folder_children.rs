...
async fn ok(env: &TestbedEnv) {
    ...
    test_register_sequence_of_send_hooks!(
        ...
        // 4) Get `/foo/egg.txt` & `/foo/spam` manifest (order of fetch is not guaranteed)
...
async fn before_workspace_manifest_v1(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history
            .stat_folder_children(wksp1_bootstrapped_timestamp, &"/".parse().unwrap())
            .await
            .unwrap_err(),
        WorkspaceHistoryStatFolderChildrenError::EntryNotFound
    );
...
async fn entry_available_but_not_referenced_in_parent(env: &TestbedEnv) {
    ...
    p_assert_eq!(
        ops.history
            .stat_folder_children(wksp1_foo_v1_timestamp, &"/".parse().unwrap())
            .await
            .unwrap(),
        vec![]
    );
    p_assert_matches!(
        ops.history
            .stat_folder_children(wksp1_foo_v1_timestamp, &"/foo".parse().unwrap())
            .await,
...
async fn entry_available_but_not_its_children(env: &TestbedEnv) {
    ...
    test_register_sequence_of_send_hooks!(
        ...
        // 4) Get `/foo/egg.txt` and `/foo/spam` manifest, which didn't exist at that time.
    ...
    p_assert_eq!(
        ops.history
            .stat_folder_children(wksp1_foo_v2_timestamp, &"/foo".parse().unwrap())
            .await
            .unwrap(),
        vec![]
    );
...
async fn offline(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history
            .stat_folder_children(at, &"/".parse().unwrap())
            .await
            .unwrap_err(),
...
async fn entry_not_found(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history
            .stat_folder_children(at, &"/dummy".parse().unwrap())
            .await
            .unwrap_err(),
        WorkspaceHistoryStatFolderChildrenError::EntryNotFound
    );
...
async fn entry_is_file(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        ops.history
            .stat_folder_children(at, &"/bar.txt".parse().unwrap())
            .await
            .unwrap_err(),
