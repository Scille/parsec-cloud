...
impl_events!(
    ...
    ///
    /// This event amis at providing feedback about the upload of the blocks of the file
    /// (hence it never occurs for folders or when a file is truncated or empty, as in
    /// those case only the manifest is uploaded which should be fast enough and is a
    /// single operation anyway so started & done events are enough).
    ///
    /// Also note the the progress may become misleading if the synchronization only
    /// upload a subset of the file (e.g. a small modification in the file leads to
    /// modifications in a single block), given in this case only the block index to
    ...
    /// This event is fired by the workspace ops when a started outbound sync cannot
    /// be achieved. This is typically the case if an inbound sync is needed first.
