...
pub enum WorkspaceSaveAndSyncFileWithOrigin {
...
pub async fn save_and_sync_file_with_origin(
    ...
    .map_err(|err| match err {
        WorkspaceOpenFileError::Offline(e) => WorkspaceSaveAndSyncFileWithOrigin::Offline(e),
        WorkspaceOpenFileError::Stopped => WorkspaceSaveAndSyncFileWithOrigin::Stopped,
        WorkspaceOpenFileError::ReadOnlyRealm => WorkspaceSaveAndSyncFileWithOrigin::ReadOnlyRealm,
        WorkspaceOpenFileError::NoRealmAccess => WorkspaceSaveAndSyncFileWithOrigin::NoRealmAccess,
        WorkspaceOpenFileError::RealmDeleted => WorkspaceSaveAndSyncFileWithOrigin::RealmDeleted,
        WorkspaceOpenFileError::EntryNotFound => WorkspaceSaveAndSyncFileWithOrigin::EntryNotFound,
        WorkspaceOpenFileError::EntryNotAFile { entry_id } => {
            WorkspaceSaveAndSyncFileWithOrigin::EntryNotAFile { entry_id }
        }
        ...
        WorkspaceOpenFileError::InvalidKeysBundle(err) => {
            WorkspaceSaveAndSyncFileWithOrigin::InvalidKeysBundle(err)
        }
        WorkspaceOpenFileError::InvalidCertificate(err) => {
            WorkspaceSaveAndSyncFileWithOrigin::InvalidCertificate(err)
        }
        WorkspaceOpenFileError::InvalidManifest(err) => {
            WorkspaceSaveAndSyncFileWithOrigin::InvalidManifest(err)
        }
    ...
    let write_outcome: Result<u64, WorkspaceSaveAndSyncFileWithOrigin> =
    ...
    let close_outcome = super::fd_close(ops, fd).await.map_err(|err| match err {
        WorkspaceFdCloseError::Stopped => WorkspaceSaveAndSyncFileWithOrigin::Stopped,
    ...
    loop {
        ...
        match outcome {
            ...
            OutboundSyncOutcome::EntryIsUnreachable => {
                return Err(WorkspaceSaveAndSyncFileWithOrigin::EntryNotFound)
            }
...
fn map_sync_error(err: WorkspaceSyncError) -> WorkspaceSaveAndSyncFileWithOrigin {
    match err {
        WorkspaceSyncError::Offline(e) => WorkspaceSaveAndSyncFileWithOrigin::Offline(e),
        WorkspaceSyncError::ServerBlockstoreUnavailable => {
            WorkspaceSaveAndSyncFileWithOrigin::ServerBlockstoreUnavailable
        }
        WorkspaceSyncError::Stopped => WorkspaceSaveAndSyncFileWithOrigin::Stopped,
        WorkspaceSyncError::NotAllowed => WorkspaceSaveAndSyncFileWithOrigin::NoRealmAccess,
        WorkspaceSyncError::NoKey => WorkspaceSaveAndSyncFileWithOrigin::NoKey,
        WorkspaceSyncError::NoRealm => WorkspaceSaveAndSyncFileWithOrigin::NoRealm,
        WorkspaceSyncError::RealmArchived => WorkspaceSaveAndSyncFileWithOrigin::RealmArchived,
        WorkspaceSyncError::RealmDeleted => WorkspaceSaveAndSyncFileWithOrigin::RealmDeleted,
        WorkspaceSyncError::InvalidManifest(err) => {
            WorkspaceSaveAndSyncFileWithOrigin::InvalidManifest(err)
        }
        WorkspaceSyncError::InvalidBlockAccess(err) => {
            WorkspaceSaveAndSyncFileWithOrigin::InvalidBlockAccess(err)
        }
        ...
        } => WorkspaceSaveAndSyncFileWithOrigin::TimestampOutOfBallpark {
            server_timestamp,
            client_timestamp,
            ballpark_client_early_offset,
            ballpark_client_late_offset,
