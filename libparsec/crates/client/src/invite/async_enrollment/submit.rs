...
pub enum SubmitAsyncEnrollmentError {
    ...
    #[error("No response from OpenBao server: {0}")]
    OpenBaoServerOffline(anyhow::Error),

    #[error("OpenBao server has rejected our request: {0}")]
    BadOpenBaoAuth(anyhow::Error),

    // TODO: add PKI-related errors
    #[error(transparent)]
    Internal(#[from] anyhow::Error),
}

#[derive(Debug, thiserror::Error)]
pub enum SubmitAsyncEnrollmentGenerateCiphertextKeyError {
    #[error("Cannot communicate with the server: {0}")]
    Offline(#[from] ConnectionError),
}

pub trait SubmitAsyncEnrollmentIdentityStrategy {
...
pub async fn submit_async_enrollment(
    ...
    let cmds = Arc::new(
        AnonymousCmds::new(&config.config_dir, addr, config.proxy.clone())
    ...
    .dump()
    .into();
    let submit_payload_signature = identity_strategy.sign(&submit_payload);
