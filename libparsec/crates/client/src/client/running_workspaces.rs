...
struct RunningWorkspace {
    ...
    monitors: Option<RunningWorkspaceMonitors>,
}
...
impl<'a> WorkspacesForUpdateGuard<'a> {
    ...
    pub async fn start_monitors_and_register(
        ...
        let monitors = match self.with_background_monitors {
            false => None,
            true => {
                let inbound_sync =
                    inbound_sync_monitor_factory(event_bus.clone(), ops.clone()).await;
                let outbound_sync =
                    outbound_sync_monitor_factory(event_bus, device, ops.clone()).await;
                Some(RunningWorkspaceMonitors {
                    inbound_sync,
                    outbound_sync,
                })
            }
        ...
        items.push(RunningWorkspace { ops, monitors })
    }

    async fn stop_workspace_monitors(started_workspace: RunningWorkspace) -> Arc<WorkspaceOps> {
        if let Some(monitors) = started_workspace.monitors {
            let RunningWorkspaceMonitors {
                mut inbound_sync,
                mut outbound_sync,
            } = monitors;
...
impl RunningWorkspaces {
    pub fn new(with_background_monitors: bool) -> Self {
        Self {
            refresh_lock: AsyncMutex::default(),
            items: Mutex::default(),
            with_background_monitors,
        }
    }
    ...
    pub async fn for_update(&self) -> WorkspacesForUpdateGuard {
        ...
        WorkspacesForUpdateGuard {
            refresh_guard,
            items: &self.items,
            with_background_monitors: self.with_background_monitors,
        }
    }
}
