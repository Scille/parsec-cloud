...
impl CertificatesOps {
    pub(super) async fn add_certificates_batch<'a>(
        ...
        for (serialized, certificate_index) in certificates.zip(last_index + 1..) {
            ...
            let any_cooked = self
                .validate_certificate(storage, certificate_index, serialized.clone())
                .await
            ...
            if let AnyArcCertificate::UserUpdate(update) = &any_cooked {
                if update.user_id == *self.device.user_id() {
    ...
    async fn validate_certificate<'a>(
            ...
            (@internal, DeviceOrRoot, $unsecure:ident) => {
                match $unsecure.author() {
                    CertificateSignerOwned::Root => $unsecure
                        .verify_signature(self.device.root_verify_key())
                        .map_err(|(unsecure, error)| {
                            let hint = unsecure.hint();
                            InvalidCertificateError::Corrupted { hint, error }
                        }),
        ...
        match unsecure {
            UnsecureAnyCertificate::User(unsecure) => {
                ...
                self.check_user_certificate_consistency(storage, current_index, &cooked)
                    .await?;
            ...
            UnsecureAnyCertificate::Device(unsecure) => {
                ...
                // 4) The certificate is valid, last check is the consistency with other certificates
                self.check_device_certificate_consistency(storage, current_index, &cooked)
                    .await?;
            ...
            UnsecureAnyCertificate::RevokedUser(unsecure) => {
                ...
                // 4) The certificate is valid, last check is the consistency with other certificates
                self.check_revoked_user_certificate_consistency(storage, current_index, &cooked)
                    .await?;
            ...
            UnsecureAnyCertificate::UserUpdate(unsecure) => {
                ...
                // 4) The certificate is valid, last check is the consistency with other certificates
                self.check_user_update_certificate_consistency(storage, current_index, &cooked)
                    .await?;
            ...
            UnsecureAnyCertificate::RealmRole(unsecure) => {
                ...
                // 4) The certificate is valid, last check is the consistency with other certificates
                self.check_realm_role_certificate_consistency(storage, current_index, &cooked)
                    .await?;
            ...
            UnsecureAnyCertificate::SequesterAuthority(unsecure) => {
                ...
                // 4) The certificate is valid, last check is the consistency with other certificates
                self.check_sequester_authority_certificate_consistency(
                    storage,
                    current_index,
                    &cooked,
                )
            ...
            UnsecureAnyCertificate::SequesterService(unsecure) => {
                    ...
                    .await
                {
                    ...
                    Err(GetCertificateError::ExistButTooRecent {
                        ...
                        let org = self.device.organization_id();
                ...
                // 4) The certificate is valid, last check is the consistency with other certificates
                self.check_sequester_service_certificate_consistency(
                    storage,
                    current_index,
                    &cooked,
                )
    ...
    async fn check_author_not_revoked_and_profile<'a>(
        ...
        if author_must_be_admin {
            match self
                .get_user_profile(storage, up_to_index, author, mk_hint, None)
                .await
            {
                Ok(profile) => {
    ...
    async fn get_user_profile<'a>(
        ...
        let user_certificate = match already_fetched_user_certificate {
            ...
            None => {
                self.check_user_exists(storage, up_to_index, user_id, mk_hint)
                    .await?
            }
        };
    ...
    async fn check_revoked_user_certificate_consistency<'a>(
        ...
        self.check_user_exists(storage, current_index, &cooked.user_id, mk_hint)
            .await?;

        // 4) Make sure the user is not already revoked

        self.check_user_not_revoked(storage, current_index, &cooked.user_id, mk_hint)
            .await?;
    ...
    async fn check_user_update_certificate_consistency<'a>(
        ...
        let user_certificate = self
            .check_user_exists(storage, current_index, &cooked.user_id, mk_hint)
            .await?;
                ...
                match certif.role {
                    ...
                    Some(RealmRole::Owner) => {
                        ...
                        if roles.len() != 1 {
                            let hint = mk_hint();
                            let what =
                                InvalidCertificateError::CannotDowngradeUserToOutsider { hint };
                            return Err(AddCertificateError::InvalidCertificate(what));
                        }
                    }
    ...
    async fn check_device_certificate_consistency<'a>(
        ...
        self.check_user_exists(storage, current_index, user_id, mk_hint)
            .await?;

        // 3) Make sure the user is not already revoked

        self.check_user_not_revoked(storage, current_index, user_id, mk_hint)
            .await?;
        ...
        match profile {
            ...
            UserProfile::Outsider => {
                ...
                if !realm_current_roles.is_empty() {
                    let hint = mk_hint();
                    let what =
                        InvalidCertificateError::RealmOutsiderCannotBeOwnerOrManager { hint };
                    return Err(AddCertificateError::InvalidCertificate(what));
                }
            }
        }
