...
#[derive(Debug, thiserror::Error)]
pub enum DataAccessGetWorkspaceManifestV1TimestampError {
    #[error("Cannot reach the server")]
    Offline,
    #[error("Component has stopped")]
    Stopped,
...
impl From<ConnectionError> for DataAccessGetWorkspaceManifestV1TimestampError {
...
impl DataAccess {
    ...
    pub async fn get_workspace_manifest_v1_timestamp(
        ...
        match self {
            DataAccess::RealmExport(data_access) => {
                data_access.get_workspace_manifest_v1_timestamp().await
            }
            DataAccess::Server(data_access) => {
                data_access.get_workspace_manifest_v1_timestamp().await
            }
        }
    }
