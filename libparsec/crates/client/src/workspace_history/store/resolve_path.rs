...
pub(crate) type WorkspaceHistoryStoreResolvePathError = DataAccessFetchManifestError;
...
fn cache_only_path_resolution(
    ...
    enum StepKind<'a> {
        Root,
        Child(&'a ArcChildManifest),
    }
    let cache_resolutions = match cache.resolutions.get(&at) {
        Some(cache_resolutions) => cache_resolutions,
        None => return CacheOnlyPathResolutionOutcome::NeedPopulateCache(store.realm_id),
    };
    ...
    loop {
        let child_name = match path_parts.get(parts_index) {
            ...
            None => match last_step {
                ...
                StepKind::Root => {
                    return match cache_resolutions.get(&store.realm_id) {
                        ...
                        None => CacheOnlyPathResolutionOutcome::NeedPopulateCache(store.realm_id),
                    };
                }
            },
        };
        ...
        let parent_manifest = match &last_step {
            StepKind::Root => match cache_resolutions.get(&store.realm_id) {
                Some(CacheResolvedEntry::Exists(ArcChildManifest::Folder(manifest))) => manifest,
                // The root is always a folder
                Some(CacheResolvedEntry::Exists(ArcChildManifest::File(_))) => unreachable!(),
                Some(CacheResolvedEntry::NotFound) => {
            ...
            StepKind::Child(manifest) => match &manifest {
                ...
                ArcChildManifest::File(_) => {
                    // Cannot continue to resolve the path !
                    return CacheOnlyPathResolutionOutcome::EntryNotFound;
                }
            },
        };
        let parent_id = parent_manifest.id;
