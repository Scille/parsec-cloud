...
#[derive(Debug, thiserror::Error)]
pub enum ServerFetchVersionsRemoteManifestError {
    #[error("Component has stopped")]
    Stopped,
    #[error("Cannot reach the server")]
    Offline,
...
impl From<ConnectionError> for ServerFetchVersionsRemoteManifestError {
    fn from(value: ConnectionError) -> Self {
        match value {
            ConnectionError::NoResponse(_) => Self::Offline,
            err => Self::Internal(err.into()),
...
pub(crate) async fn server_fetch_versions_remote_workspace_manifest(
    cmds: &AuthenticatedCmds,
    certificates_ops: &CertificateOps,
    realm_id: VlobID,
    versions: &[VersionInt],
) -> Result<Vec<FolderManifest>, ServerFetchVersionsRemoteManifestError> {
    ...
    for (_, key_index, expected_author, expected_version, expected_timestamp, blob) in items {
        let manifest = certificates_ops
            ...
            .await
            .map_err(|err| match err {
                CertifValidateManifestError::Offline => {
                    ServerFetchVersionsRemoteManifestError::Offline
                }
                CertifValidateManifestError::Stopped => {
...
async fn fetch_versions_vlob(
...
) -> Result<VlobVersionsData, ServerFetchVersionsRemoteManifestError> {
    ...
    match rep {
        ...
        // Expected errors
        Rep::AuthorNotAllowed => Err(ServerFetchVersionsRemoteManifestError::NoRealmAccess),
        Rep::RealmNotFound => Err(ServerFetchVersionsRemoteManifestError::RealmNotFound),
        // Unexpected errors :(
        rep @ (
            // One item is too many ???? Really ????
            Rep::TooManyElements |
