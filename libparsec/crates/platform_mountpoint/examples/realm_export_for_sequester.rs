...
async fn main() -> ExitCode {
    ...
    let timestamp_of_interest = match args.next() {
        ...
        // By default, provide a timestamp where part of the workspace data has been
        // created after the realm key rotation following sequester service 1 revocation
        // (see `libparsec/crates/testbed/src/templates/sequestered.rs` documentation).
        None => "2001-01-30T00:00:00Z".parse().unwrap(),
    };
    ...
    // Retreive the path of the realm export dump

    let exe_path = std::env::current_exe().unwrap();
    let mut path: &Path = exe_path.as_ref();
    let export_db_path = loop {
        if path.ends_with("target") {
            break path.join("../server/tests/sequester_export/sample_export.sqlite");
        }
        match path.parent() {
