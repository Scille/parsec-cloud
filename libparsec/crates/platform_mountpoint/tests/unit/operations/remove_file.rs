...
async fn ok(tmp_path: TmpPath, env: &TestbedEnv) {
    mount_and_test!(
        ...
        |_client: Arc<Client>, wksp1_ops: Arc<WorkspaceOps>, mountpoint_path: PathBuf| async move {
            p_assert_matches!(
                wksp1_ops.stat_entry(&"/foo/egg.txt".parse().unwrap()).await,
                Ok(_)
            );
...
#[parsec_test(testbed = "minimal_client_ready")]
async fn is_folder(tmp_path: TmpPath, env: &TestbedEnv) {
    mount_and_test!(
        env,
        &tmp_path,
        |_client: Arc<Client>, _wksp1_ops: Arc<WorkspaceOps>, mountpoint_path: PathBuf| async move {
            ...
            let err = tokio::fs::remove_file(&path).await.unwrap_err();
