...
use libparsec_client::{
    workspace_history::WorkspaceHistoryOps,
    Client,
};
...
async fn ok(
    ...
    let wksp1_v2_timestamp: DateTime = *env
        .template
        .get_stuff("wksp1_v2_timestamp");
    ...
    mount_history_and_test!(
        ...
        |_client: Arc<Client>, _wksp1_ops: Arc<WorkspaceHistoryOps>, mountpoint_path: PathBuf| async move {
            ...
            let err = tokio::task::spawn_blocking(move || {
                std::fs::OpenOptions::new().read(true).open(mountpoint_path.join(name)).unwrap_err()
            })
            .await.unwrap();
