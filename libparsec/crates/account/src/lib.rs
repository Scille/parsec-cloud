...
pub async fn account_create_proceed(
    ...
    let req_step = match step {
        ...
        AccountCreateStep::Create {
            ...
            let auth_method_password_algorithm = PasswordAlgorithm::Argon2id {
                salt: salt.to_vec(),
                opslimit,
                memlimit_kb,
                parallelism,
            ...
            let auth_method_master_secret_key =
                auth_method_password_algorithm.compute_secret_key(&password)?;

            let auth_method_master_secret =
                auth_method_password_algorithm.compute_key_derivation(&password)?;
