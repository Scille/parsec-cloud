...
use crate::{Account, AccountCreateWebLocalDeviceKeyError};

#[parsec_test(testbed = "minimal", with_server)]
async fn ok_with_server(env: &TestbedEnv) {
    let alice = env.local_device("alice@dev1");
    let web_local_device_key = SecretKey::generate();
    ...
    account
        .get_web_local_device_key(alice.organization_id(), alice.device_id)
        .await
        .unwrap();
}
...
async fn ok_mocked(env: &TestbedEnv) {
    ...
    test_register_sequence_of_send_hooks!(
        ...
        test_send_hook_vault_item_list!(env, &account.auth_method_secret_key, &vault_key),
        {
            ...
            let web_local_device_key = web_local_device_key.clone();
            ...
            move |req: authenticated_account_cmds::latest::vault_item_upload::Req| {
                ...
                match item {
                    AccountVaultItem::WebLocalDeviceKey(item) => {
                        ...
                        let key_access = WebLocalDeviceKeyAccess::decrypt_and_load(
                            &item.encrypted_data,
                            &vault_key,
                        )
                        .unwrap();
                        p_assert_eq!(key_access.web_local_device_key, web_local_device_key);
                    }
...
async fn offline(
    ...
    p_assert_matches!(
        account
            ...
            .await,
        Err(AccountCreateWebLocalDeviceKeyError::Offline(_))
    );
}
...
async fn fingerprint_already_exists(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        account.create_web_local_device_key(alice.organization_id().to_owned(), alice.device_id, SecretKey::generate()).await,
        Err(AccountCreateWebLocalDeviceKeyError::Internal(err))
...
async fn bad_vault_key_access(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        account
            ...
            .await,
        Err(AccountCreateWebLocalDeviceKeyError::BadVaultKeyAccess(_))
    );
}
