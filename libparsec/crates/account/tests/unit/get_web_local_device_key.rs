...
use crate::{Account, AccountGetWebLocalDeviceKeyError};
...
async fn ok(env: &TestbedEnv) {
    ...
    let alice_web_local_device_key = SecretKey::generate();
    ...
    test_register_sequence_of_send_hooks!(
        ...
        test_send_hook_vault_item_list!(env, &account.auth_method_secret_key, &vault_key, web_local_device_keys: [
            (&alice, alice_web_local_device_key),
...
async fn bad_vault_key_access(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        account
            .get_web_local_device_key(alice.organization_id(), alice.device_id)
            .await,
        Err(AccountGetWebLocalDeviceKeyError::BadVaultKeyAccess(_))
    );
}
...
async fn offline(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        account
            .get_web_local_device_key(alice.organization_id(), alice.device_id)
            .await,
        Err(AccountGetWebLocalDeviceKeyError::Offline(_))
    );
}
...
async fn unknown_server_response(env: &TestbedEnv) {
    ...
    p_assert_matches!(
        account.get_web_local_device_key(alice.organization_id(), alice.device_id).await,
        Err(AccountGetWebLocalDeviceKeyError::Internal(err))
