...
use crate::{
    Account, AccountAuthMethodCreateError, AccountAuthMethodStrategy, AccountLoginStrategy,
};
...
async fn ok(#[values("master_secret", "password")] kind: &str, env: &TestbedEnv) {
    ...
    account
        .auth_method_create(auth_method_strategy)
        .await
        .unwrap();
...
async fn auth_method_id_already_exists(env: &TestbedEnv) {
    ...
    let result = account
        .auth_method_create(AccountAuthMethodStrategy::MasterSecret(&master_secret))
        .await;

    p_assert_matches!(result, Err(AccountAuthMethodCreateError::Internal(err))
