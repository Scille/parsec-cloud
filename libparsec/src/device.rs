...
pub async fn update_device_change_authentication(
    ...
    let new_auth = new_auth.convert_with_side_effects().map_err(UpdateDeviceError::Internal)?;
...
pub async fn update_device_overwrite_server_addr(
    ...
    let auth = auth.convert_with_side_effects().map_err(UpdateDeviceError::Internal)?;
