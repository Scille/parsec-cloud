...
pub struct OpenBaoConfig {
    ...
    pub auths: Vec<OpenBoaAuthConfig>,
}
                ...
                libparsec_protocol::anonymous_server_cmds::v5::server_config::OpenBaoConfig::Enabled { auths, secret, server_url } => Some(
                    OpenBaoConfig {
                        ...
                        auths: auths.into_iter().filter_map(|auth| match auth.id.as_ref() {
                            "OIDC_HEXAGONE" => Some(OpenBoaAuthConfig::OIDCHexagone { mount_path: auth.mount_path }),
                            "OIDC_PRO_CONNECT" => Some(OpenBoaAuthConfig::OIDCProConnect { mount_path: auth.mount_path }),
