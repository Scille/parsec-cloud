...
pub use libparsec_client::workspace::{
    ...
    WorkspaceRemoveEntryError, WorkspaceSaveAndSyncFileWithOrigin, WorkspaceStatEntryError,
...
pub type WorkspaceSaveAndSyncFileWithCryptpad = WorkspaceSaveAndSyncFileWithOrigin;
...
pub async fn workspace_save_and_sync_file_with_cryptpad(
    ...
    content: &[u8],
) -> Result<(), WorkspaceSaveAndSyncFileWithCryptpad> {
    let workspace = borrow_workspace(workspace)?;
