...
class MemoryUserComponent(BaseUserComponent):
    ...
    async def cancel_user_invitation(
        ...
        if org.invitations.pop(user_id, None):
            await self._send_event(
                BackendEvents.user_invitation_cancelled,
                organization_id=organization_id,
                user_id=user_id,
            )
    ...
    async def cancel_device_invitation(
        ...
        if org.invitations.pop(device_id, None):
            await self._send_event(
                BackendEvents.device_invitation_cancelled,
                organization_id=organization_id,
                device_id=device_id,
            )
    ...
    async def revoke_user(
        ...
        await self._send_event(
            BackendEvents.user_revoked, organization_id=organization_id, user_id=user_id
        )
