...
class MemoryRealmComponent(BaseRealmComponent):
    ...
    async def get_realms_history_for_user(self, organization_id: OrganizationID, user: UserID) -> List[Tuple[DateTime, RealmID, DeviceID, RealmRole]]:
        history = []
        for (realm_org_id, realm_id), realm in self._realms.items():
            if realm_org_id != organization_id:
                continue
            ...
            for granted_role in realm.granted_roles:
                if granted_role.user_id != user:
                    continue

                history.append((granted_role.granted_on, realm_id, granted_role.granted_by, granted_role.role))
