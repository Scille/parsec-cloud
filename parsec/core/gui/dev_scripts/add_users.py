...
# users (in what I'm sure is the best way possible :p) to see how the GUI handles user
...
def add_user(password, device, prefix):
    try:
        username = prefix + "".join(random.choice(string.ascii_lowercase) for _ in range(8))
        token = core_call().invite_user(username)
        privkey, signkey, manifest = core_call().claim_user(username, device, token)
        privkey = privkey.encode()
        signkey = signkey.encode()
        device_id = f"{username}@{device}"
        core_call().register_new_device(device_id, privkey, signkey, manifest, password, False)
    except:
        print("Failed to add user")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add N users")
    parser.add_argument("-n", type=int, default=20, help="Number of users to add")
    parser.add_argument(
        "-p", "--password", type=str, default="", help="Default password for the users"
    )
    parser.add_argument(
        "--prefix", type=str, default="user_", help="Prefix for generated user names"
    )
    parser.add_argument("-d", "--device", default="device", help="Default device name")
    args = parser.parse_args()

    start()

    for i in range(args.n):
        add_user(args.password, args.device, args.prefix)
