...
class FileSyncerMixin(BaseSyncer):
        ...
        async def _process_buffer():
            while buffers:
                ...
                if isinstance(bs.buffer, BlockBuffer):
                    ...
                    buff = self.local_storage.get_block(bs.buffer.access.id)
                assert buff
    ...
    def _sync_file_look_resolve_concurrency(
        ...
        moved_entry_access = ManifestAccess(
            moved_entry_id, access.realm_id, access.key, access.encryption_revision
        )
        self.local_folder_fs.set_dirty_manifest(moved_entry_access, diverged_manifest)
        self.local_folder_fs.set_dirty_manifest(parent_access, parent_manifest)
        self.local_storage.set_base_manifest(access.id, target_remote_manifest)
        ...
        self.event_bus.send(
            ...
            diverged_id=moved_entry_id,
        )
        self.event_bus.send("fs.entry.updated", id=moved_entry_id)
    ...
    async def _sync_file_actual_sync(
        ...
        async def _process_spaces():
            ...
            while spaces:
                ...
                if not data:
                    ...
                    await self._backend_block_create(access, block_access, data)
                    blocks.append(block_access)

                    # The block has been successfully uploaded
                    # Keep it in the remote storage
                    self.local_storage.set_clean_block(block_access.id, data)
        ...
        try:
            ...
            logger.warning("Error while synchronizing a non-referenced file", entry_id=access.id)
            return
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Error while synchronizing a non-referenced file", entry_id=access.id
                )
                return

            # The vlob has been successfully uploaded - clean up the dirty blocks
            for dirty_block in manifest.dirty_blocks:
                self.local_storage.clear_dirty_block(dirty_block.id)
            ...
            for remote_block in outdated_blocks:
                self.local_storage.clear_clean_block(remote_block.id)
    ...
    def _sync_file_merge_back(
        ...
        if final_manifest.need_sync:
            ...
            self.local_storage.set_base_manifest(access.id, final_manifest.to_remote())
