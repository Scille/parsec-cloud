...
class BaseClaimInProgress1Ctx:
    ...
    def generate_greeter_sas_choices(self, size: int = 3) -> List[int]:
...
class UserClaimInProgress3Ctx:
    ...
    async def do_claim_user(
        ...
        if rep["payload"] is None:
            raise InviteError("Missing InviteUserConfirmation payload")
...
class DeviceClaimInProgress3Ctx:
    ...
    async def do_claim_device(self, requested_device_name: DeviceName) -> LocalDevice:
        ...
        if rep["payload"] is None:
            raise InviteError("Missing InviteDeviceConfirmation payload")
