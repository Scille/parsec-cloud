...
def _retrieve_by_import(path):
    ...
    try:
        ...
        raise ConfigError("Unknown ressource `%s`." % path)


def _check_fields(data, required=None, optional=None):
    errors = []
...
def _build_obj_from_config(data):
    """
    Config example::
    ...
    """
    _check_fields(data, required={'class': str}, optional={'id': str, 'params': dict})
    cls = _retrieve_by_import(data['class'])
    raw_params = data.get('params', {})
    params = _cook_params_from_annotations(cls.__init__, raw_params)
    return LazyObj(data.get('id', uuid4().hex), cls, **params)


def _cook_params_from_annotations(builder, params):
        ...
        if t in (int, float, str, list, dict):
            ...
            try:
                ...
                if isinstance(v, dict):
                    ...
                    cooked[k] = _build_obj_from_config(v)
...
def _parse_list_objs(config_list):
    objs = []
    errors = {}
    for i, obj_config in enumerate(config_list):
        try:
            objs.append(_build_obj_from_config(obj_config))
        except ConfigError as exc:
            errors[i] = exc.errors
    if errors:
        raise ConfigError(errors)
...
def loads_config(raw_data):
    ...
    errors = {}
    try:
        for service in _parse_list_objs(raw_yaml.get('services')):
            topology.add_service(service.id, service)
    except ConfigError as exc:
        errors['services'] = exc.errors
    try:
        for client in _parse_list_objs(raw_yaml.get('clients')):
            topology.add_client(client.id, client)
    except ConfigError as exc:
        errors['clients'] = exc.errors
    try:
        for server in _parse_list_objs(raw_yaml.get('servers')):
