...
async def totp_fetch_opaque_key(
    ...
    if opaque_key_failed_attempts > 0:
        assert opaque_key_last_failed_attempt is not None, row
        wait_until = opaque_key_last_failed_attempt.add(
            seconds=2 ** (opaque_key_failed_attempts - 1)
        )
        if now <= wait_until:
