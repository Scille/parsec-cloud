...
async def get_sequester_services(
    conn: asyncpg.Connection, organization_id: OrganizationID, with_disabled: bool
...
class PGSequesterComponent(BaseSequesterComponent):
    ...
    async def dump_realm(
        ...
        async with self.pool.acquire() as conn:
            ...
            # It appears that asyncpg does not handle "WHERE vlob_id IN ($vlob_ids)" queries if items are UUIDs
