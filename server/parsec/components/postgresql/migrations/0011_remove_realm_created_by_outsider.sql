...
clean_realm_user_role AS (
    ...
    RIGHT JOIN to_remove_realm
    ON realm_user_role.realm = to_remove_realm.realm
),

clean_realm_archiving AS (
    DELETE FROM realm_archiving
    RIGHT JOIN to_remove_realm
    ON realm_archiving.realm = to_remove_realm.realm
),

clean_realm_user_change AS (
    DELETE FROM realm_user_change
    RIGHT JOIN to_remove_realm
    ON realm_user_change.realm = to_remove_realm.realm
),

clean_realm_keys_bundle AS (
    DELETE FROM realm_keys_bundle
    RIGHT JOIN to_remove_realm
    ON realm_keys_bundle.realm = to_remove_realm.realm
),

clean_realm_keys_bundle_access AS (
    DELETE FROM realm_keys_bundle_access
    RIGHT JOIN to_remove_realm
    ON realm_keys_bundle_access.realm = to_remove_realm.realm
),

clean_realm_sequester_keys_bundle_access AS (
    DELETE FROM realm_sequester_keys_bundle_access
    RIGHT JOIN to_remove_realm
    ON realm_sequester_keys_bundle_access.realm = to_remove_realm.realm
),

clean_realm_name AS (
    DELETE FROM realm_name
    RIGHT JOIN to_remove_realm
    ON realm_name.realm = to_remove_realm.realm
),

clean_vlob_atom AS (
    DELETE FROM vlob_atom
    RIGHT JOIN to_remove_realm
    ON vlob_atom.realm = to_remove_realm.realm
),

clean_realm_vlob_update AS (
    DELETE FROM realm_vlob_update
    RIGHT JOIN to_remove_realm
    ON realm_vlob_update.realm = to_remove_realm.realm
),

clean_block AS (
    DELETE FROM block
    RIGHT JOIN to_remove_realm
    ON block.realm = to_remove_realm.realm
),

clean_realm_topic AS (
    DELETE FROM realm_topic
    RIGHT JOIN to_remove_realm
    ON realm_topic.realm = to_remove_realm.realm
)

-- Clean the realm table
DELETE FROM realm
RIGHT JOIN to_remove_realm
ON realm._id = to_remove_realm.realm
;
