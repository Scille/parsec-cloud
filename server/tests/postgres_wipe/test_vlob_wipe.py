...
async def test_vlob_cascade_wipe(coolorg: CoolorgRpcClients, backend: Backend):
    ...
    async with vlob.pool.acquire() as conn:
        ...
        vlob_atom_ids = await fetch_many_row_ids_from_references(
            conn, "vlob_atom", "realm", realm_ids
        )
        assert vlob_atom_ids

        realm_vlob_update_ids = await fetch_many_row_ids_from_references(
            conn, "realm_vlob_update", "realm", realm_ids
        )
    ...
    async with vlob.pool.acquire() as conn:
        for table, id in itertools.chain(
            (("vlob_atom", id) for id in vlob_atom_ids),
            (("realm_vlob_update", id) for id in realm_vlob_update_ids),
        ):
