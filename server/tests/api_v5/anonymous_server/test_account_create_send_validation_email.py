...
async def test_anonymous_server_account_create_send_validation_email_ok(
    ...
    match kind:
        ...
        case "out_of_cooldown_and_still_valid_previous":
            ...
            outcome = await backend.account.create_send_validation_email(
        ...
        case "too_old_previous":
            ...
            old_validation_code = await backend.account.create_send_validation_email(
        ...
        case "out_of_cooldown_and_too_many_attempts_previous":
            ...
            while True:
                ...
                if bad_validation_code != old_validation_code:
                    break
            while True:
                outcome = await backend.account.create_check_validation_code(
                    DateTime.now(), email, bad_validation_code
                )
                match outcome:
    ...
    if old_validation_code is not None:
        outcome = await backend.account.create_check_validation_code(
            DateTime.now(), email, old_validation_code
        )
        assert outcome == AccountCreateProceedBadOutcome.INVALID_VALIDATION_CODE

    # ...but the new one is !
    outcome = await backend.account.create_check_validation_code(
        DateTime.now(), email, new_validation_code
    )
