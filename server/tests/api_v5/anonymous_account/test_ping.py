...
async def test_anonymous_account_ping_ok(
    ...
    rep = await minimalorg.anonymous_account.ping(ping="hello")
    assert rep == anonymous_account_cmds.latest.ping.RepOk(pong="hello")


async def test_anonymous_account_ping_http_common_errors(
    coolorg: CoolorgRpcClients, anonymous_account_http_common_errors_tester: HttpCommonErrorsTester
) -> None:
    async def do():
        await coolorg.anonymous_account.ping(ping="hello")
