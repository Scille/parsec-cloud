...
impl WorkspaceStorage {
    ...
    fn close_connections(&self) -> FutureIntoCoroutine {
        ...
        FutureIntoCoroutine::from_safe(async move { ws.close_connections().await })
    }
    ...
    fn clear_clean_block(&self, block_id: BlockID) -> FutureIntoCoroutine {
        ...
        FutureIntoCoroutine::from_safe(async move { ws.clear_clean_block(block_id.0).await })
    }
    ...
    fn get_realm_checkpoint(&self) -> FutureIntoCoroutine {
        ...
        FutureIntoCoroutine::from_safe(async move { ws.get_realm_checkpoint().await })
    }
    ...
    fn is_manifest_cache_ahead_of_persistance(&self, entry_id: EntryID) -> FutureIntoCoroutine {
        ...
        FutureIntoCoroutine::from_safe(async move {
            ws.is_manifest_cache_ahead_of_persistance(&entry_id.0).await
        })
    }
}
...
impl WorkspaceStorageSnapshot {
    ...
    fn clear_local_cache(&self) -> FutureIntoCoroutine {
        ...
        FutureIntoCoroutine::from_safe(async move { ws.clear_local_cache().await })
    }
