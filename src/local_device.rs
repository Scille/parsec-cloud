...
pub(crate) fn add_mod(py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(save_device_with_biometrics, m)?)?;
    m.add_function(wrap_pyfunction!(save_device_with_biometrics_in_config, m)?)?;
...
impl LocalDevice {
    ...
    #[classmethod]
    fn load_device_with_biometrics(_cls: &PyType, key_file: PathBuf) -> LocalDeviceResult<Self> {
        platform_device_loader::load_device_with_biometrics_from_path(&key_file)
            .map(|local_device| LocalDevice(Arc::new(local_device)))
            .map_err(|e| e.into())
    }
...
impl From<LocalDeviceError> for PyErr {
    fn from(value: LocalDeviceError) -> Self {
        ...
        match internal {
            ...
            | types::LocalDeviceError::Biometrics(_) => {
...
#[pyfunction]
fn save_device_with_biometrics(
    key_file: PathBuf,
    device: &LocalDevice,
    force: bool,
) -> LocalDeviceResult<()> {
    platform_device_loader::save_device_with_biometrics(&key_file, &device.0, force)
        .map_err(|e| e.into())
}

#[pyfunction]
fn save_device_with_biometrics_in_config(
    config_dir: PathBuf,
    device: &LocalDevice,
) -> LocalDeviceResult<PathBuf> {
    platform_device_loader::save_device_with_biometrics_in_config(&config_dir, &device.0)
        .map_err(|e| e.into())
}
#[pyfunction]
