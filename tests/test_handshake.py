...
def test_good_administration_handshake():
    ...
    assert sh.answer_data == {"client_api_version": __api_version__, "token": admin_token}
    result_req = sh.build_result_req()
    assert sh.state == "result"
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {
            ...
            "supported_api_versions": (__api_version__,),
        },
        {"handshake": "challenge", "challenge": b"1234567890"},
        {"challenge": b"1234567890"},
        {"challenge": b"1234567890", "supported_api_versions": (__api_version__,)},
        {"handshake": "challenge", "challenge": None},
        {"handshake": "challenge", "challenge": None, "supported_api_versions": (__api_version__,)},
        {"handshake": "challenge", "challenge": 42, "supported_api_versions": (__api_version__,)},
        {"handshake": "challenge", "challenge": b"1234567890"},
...
@pytest.mark.parametrize(
    "req",
    [
        {"handshake": "challenge", "challenge": b"1234567890", "supported_api_versions": ((0, 1),)},
        {
            "handshake": "challenge",
            "challenge": b"1234567890",
            "supported_api_versions": ((0, 100),),
        },
        {"handshake": "challenge", "challenge": b"1234567890", "supported_api_versions": ((2, 0),)},
    ],
)
def test_process_challenge_req_bad_semver(alice, req, monkeypatch):
    ...
    monkeypatch.setattr(ch, "supported_api_versions", ((1, 1),))
...
@pytest.mark.parametrize(
    "req",
    [
        {"handshake": "challenge", "challenge": b"1234567890", "supported_api_versions": ((1, 0),)},
        {"handshake": "challenge", "challenge": b"1234567890", "supported_api_versions": ((1, 1),)},
        {"handshake": "challenge", "challenge": b"1234567890", "supported_api_versions": ((1, 2),)},
        {
            "handshake": "challenge",
            "challenge": b"1234567890",
            "supported_api_versions": ((1, 100),),
        },
    ],
)
...
def test_process_answer_req_bad_format(req, alice):
    for key, good_value in [
        ...
        if req.get(key) == "<good>":
            req[key] = good_value
    req["supported_api_versions"] = (__api_version__,)
    sh = ServerHandshake()
