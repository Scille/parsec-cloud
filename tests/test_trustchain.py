...
@pytest.mark.trio
async def test_cascade_validate_devices_ok_with_user_certifier(coolorg, alice, bob, mallory):
    now = Pendulum(2000, 1, 1)
    ...
    with freeze_time(now):
        ...
        user_certification = certify_user(
            certifier_id=mallory.device_id,
            certifier_key=mallory.signing_key,
            user_id=alice.user_id,
            public_key=alice.public_key,
            now=now,
        )
        ...
        alice_user = RemoteUser(
            user_id="alice",
            certified_user=user_certification,
            user_certifier=mallory.device_id,
            devices=RemoteDevicesMapping(alice_device, bob_device, mallory_device),
            created_on=now,
        )

        trustchain = {
            DeviceID("alice@dev1"): alice_device,
            DeviceID("alice@dev2"): bob_device,
            DeviceID("alice@dev3"): mallory_device,
            mallory.device_id: mallory_device,
        }
...
@pytest.mark.trio
async def test_cascade_validate_devices_with_user_created_by_revoked_certifier(
    coolorg, alice, bob, mallory
):
    now = Pendulum(2000, 1, 1)
    ...
    with freeze_time(now):
        ...
        mallory_device_revocation = certify_device_revocation(
            certifier_id=DeviceID("alice@dev2"),
            certifier_key=bob.signing_key,
            revoked_device_id=DeviceID("alice@dev3"),
            now=Pendulum(1999, 1, 1),
        )
        ...
        mallory_device = RemoteDevice(
            ...
            device_certifier=DeviceID("alice@dev2"),
            created_on=now,
            certified_revocation=mallory_device_revocation,
            revocation_certifier=DeviceID("alice@dev1"),
            revocated_on=Pendulum(1999, 1, 1),
        )
...
@pytest.mark.trio
async def test_cascade_validate_devices_with_revoked_user(coolorg, alice, bob, mallory):
    now = Pendulum(2000, 1, 1)
    ...
    with freeze_time(now):
        ...
        alice_device_revocation = certify_device_revocation(
        ...
        alice_device = RemoteDevice(
            ...
            device_certifier=None,
            created_on=now,
            certified_revocation=alice_device_revocation,
            revocation_certifier=DeviceID("alice@dev1"),
            revocated_on=now,
        )
        ...
        alice_user = RemoteUser(
            ...
            certified_user=user_certification,
            user_certifier=None,
            devices=RemoteDevicesMapping(alice_device),
            created_on=now,
        )

        trustchain = {DeviceID("alice@dev1"): alice_device}
...
@pytest.mark.trio
async def test_cascade_validate_devices_with_valid_revocation(coolorg, alice, bob, mallory):
    now = Pendulum(2000, 1, 1)
...
@pytest.mark.trio
async def test_cascade_validate_devices_with_invalid_revocation(coolorg, alice, bob, mallory):
    now = Pendulum(2000, 1, 1)
