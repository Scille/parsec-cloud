...
from tests.core.fs.old_storage.chunk_storage import BlockStorage, ChunkStorage
from tests.core.fs.old_storage.local_database import LocalDatabase
    ...
    async with LocalDatabase.run(db_path) as data_localdb:
        ...
        async with ManifestStorage.run(device, data_localdb, workspace_id) as manifest_storage:
            ...
            manifest = LocalWorkspaceManifest.new_placeholder(
                ...
                id=workspace_id,
                timestamp=device.time_provider.now(),
                speculative=False,
            )
...
class BaseWorkspaceStorage:
    ...
    async def set_workspace_manifest(self, manifest: LocalWorkspaceManifest) -> None:
        """
        Compatibility method for old vs new
        """
        return await self.set_manifest(self.workspace_id, manifest)
...
class WorkspaceStorage(BaseWorkspaceStorage):
    ...
    async def run(
        ...
        cache_size = cache_size or DEFAULT_WORKSPACE_STORAGE_CACHE_SIZE
        data_vacuum_threshold = data_vacuum_threshold or DEFAULT_CHUNK_VACUUM_THRESHOLD
