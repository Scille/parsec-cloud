...
@pytest.mark.trio
async def test_realm_notif_maintenance_on_reencryption(running_backend, alice_core, bob_user_fs):
...
@pytest.mark.trio
async def test_realm_notif_maintenance_on_garbage_collection(
    running_backend, alice_core, bob_user_fs
):
    wid = await create_shared_workspace("w", bob_user_fs, alice_core)

    with alice_core.event_bus.listen() as spy:
        # Start maintenance
        job = await bob_user_fs.workspace_start_garbage_collection(wid)
        await spy.wait_multiple_with_timeout(
            [
                ("backend.realm.maintenance_started", {"realm_id": wid, "encryption_revision": 1}),
                # Receive and process the new key and encryption revision
                "backend.message.received",
            ]
        )
    ...
    with alice_core.event_bus.listen() as spy:
        ...
        await spy.wait_with_timeout(
            "backend.realm.maintenance_finished", {"realm_id": wid, "encryption_revision": 1}
        )
