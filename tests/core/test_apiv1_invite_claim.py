...
async def _invite_and_claim(
    running_backend, invite_func, claim_func, event_name=BackendEvents.user_claimed
):
...
async def test_invite_claim_device(running_backend, backend, alice):
    ...
    await _invite_and_claim(
        running_backend, _from_alice, _from_new_device, event_name=BackendEvents.device_claimed
    )
...
async def test_invite_claim_multiple_devices_from_chained_user(running_backend, backend, alice):
    ...
    await _invite_and_claim(running_backend, _invite_from_alice, _claim_from_1)
    await _invite_and_claim(
        running_backend, _invite_from_1, _claim_from_2, event_name=BackendEvents.device_claimed
    )
    await _invite_and_claim(
        running_backend, _invite_from_2, _claim_from_3, event_name=BackendEvents.device_claimed
    )
...
async def test_device_invite_claim_cancel_invitation(running_backend, backend, alice):
    ...
    await _invite_and_claim(
        ...
        event_name=BackendEvents.device_claimed,
    )
