...
class TestUserManifestService:
    ...
    async def test_create_dir(self, user_manifest_svc):
        # Working
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_make_dir', 'path': '/test_dir'})
    ...
    async def test_remove_dir(self, user_manifest_svc):
        ...
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_remove_dir', 'path': '/test_dir'})
        assert ret == {'status': 'ok'}
        # Not found
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_remove_dir', 'path': '/test_dir'})
    ...
    async def test_cant_remove_root_dir(self, user_manifest_svc):
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_remove_dir', 'path': '/'})
    ...
    async def test_remove_not_empty_dir(self, user_manifest_svc):
        ...
        # Empty
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_delete_file', 'path': '/test_dir/test'})
        assert ret == {'status': 'ok'}
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_remove_dir', 'path': '/test_dir'})
        assert ret == {'status': 'ok'}
    ...
    async def test_list_dir(self, user_manifest_svc):
        ...
        async def assert_ls(path, expected_childrens):
            ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_list_dir', 'path': path})
            assert ret['status'] == 'ok'
        ...
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_list_dir', 'path': '/dummy'})
        assert ret == {'status': 'not_found', 'label': 'Directory not found.'}

        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_list_dir', 'path': '/countries/dummy'})
        assert ret == {'status': 'not_found', 'label': 'Directory not found.'}
    ...
    async def test_rename_file(self, user_manifest_svc):
        ...
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_rename_file',
                                                    ...
                                                    'new_path': '/foo'})
        assert ret['status'] == 'ok'
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_delete_file', 'path': '/test'})
        assert ret == {'status': 'not_found', 'label': 'File not found.'}
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_delete_file', 'path': '/foo'})
        assert ret == {'status': 'ok'}
    ...
    async def test_delete_file(self, path, user_manifest_svc):
        ...
        await user_manifest_svc.make_dir('/test_dir')
        await user_manifest_svc.create_file('/test_dir/test')
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_delete_file', 'path': path})
        assert ret == {'status': 'ok'}
        # File not found
        ret = await user_manifest_svc.dispatch_msg({'cmd': 'user_manifest_delete_file', 'path': path})
        assert ret == {'status': 'not_found', 'label': 'File not found.'}
