...
async def test_register_user_open_modal(aqtbot, logged_gui_alice, running_backend):
    ...
    with patch("parsec.core.gui.users_widget.InviteUserWidget") as register_mock:
...
@pytest.mark.skip(
    "Useless test, since in the GUI, users have no way to open the InviteUserWidget if they are not admin"
)
@pytest.mark.gui
@pytest.mark.trio
