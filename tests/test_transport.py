...
async def test_big_buffer_bench():
    ...
    async def _boot_server():
        ...
        server_transport = await WebsocketTransport.init_for_server(server_stream)

    async def _boot_client():
        nonlocal client_transport
        client_transport = await WebsocketTransport.init_for_client(client_stream, "foo", "bar")
    ...
    roughly_max = int(WebsocketTransport.RECEIVE_BYTES * 2 / 3)
