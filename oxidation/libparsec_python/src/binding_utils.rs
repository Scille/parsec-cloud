...
pub fn py_to_rs_datetime(timestamp: &PyAny) -> PyResult<libparsec::api_types::DateTime> {
    let ts_any =
        Python::with_gil(|_py| -> PyResult<&PyAny> { timestamp.getattr("timestamp")?.call0() })?;
    let ts = ts_any.extract::<f64>()?;
    Ok(libparsec::api_types::DateTime::from_f64_with_us_precision(
        ts,
    ))
}

pub fn rs_to_py_datetime(py: Python, datetime: libparsec::api_types::DateTime) -> PyResult<&PyAny> {
...
pub fn rs_to_py_realm_role(role: &libparsec::api_types::RealmRole) -> PyResult<PyObject> {
    Python::with_gil(|py| -> PyResult<PyObject> {
        let cls = py.import("parsec.api.protocol")?.getattr("RealmRole")?;
        let role_name = match role {
            libparsec::api_types::RealmRole::Owner => "OWNER",
            libparsec::api_types::RealmRole::Manager => "MANAGER",
            libparsec::api_types::RealmRole::Contributor => "CONTRIBUTOR",
            libparsec::api_types::RealmRole::Reader => "READER",
        };
...
pub fn py_to_rs_realm_role(role: &PyAny) -> PyResult<Option<libparsec::api_types::RealmRole>> {
    if role.is_none() {
        return Ok(None);
    }
    use libparsec::api_types::RealmRole::*;
...
pub fn py_to_rs_user_profile(profile: &PyAny) -> PyResult<libparsec::api_types::UserProfile> {
    use libparsec::api_types::UserProfile::*;
...
pub fn rs_to_py_user_profile(profile: &libparsec::api_types::UserProfile) -> PyResult<PyObject> {
    use libparsec::api_types::UserProfile::*;
    Python::with_gil(|py| -> PyResult<PyObject> {
...
pub fn py_to_rs_invitation_status(
    status: &PyAny,
) -> PyResult<libparsec::api_types::InvitationStatus> {
    use libparsec::api_types::InvitationStatus::*;
