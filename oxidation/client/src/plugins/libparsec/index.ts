...
const libparsec_plugin = registerPlugin<LibParsecPlugin>(
  "LibParsec",
  {
...
class LibParsec {
  ...
  public async encrypt(key: string, cleartext: string): Promise<string> {
    const ret = await libparsec_plugin.submitJob({cmd: "encrypt", payload: `${key}:${cleartext}`})
    return ret.value
  }


  public async decrypt(key: string, cyphertext: string): Promise<string> {
    const ret = await libparsec_plugin.submitJob({cmd: "decrypt", payload: `${key}:${cyphertext}`})
    return ret.value
  }
