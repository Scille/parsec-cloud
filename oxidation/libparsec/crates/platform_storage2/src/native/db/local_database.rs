...
use super::option::{AutoVacuum, PragmaJournalMode, PragmaOptions, PragmaSynchronous};
...
impl LocalDatabase {
    ...
    pub async fn from_path(
        ...
        let conn = conn.or_else(|| super::in_memory_mock::maybe_open_sqlite_in_memory(&path_info));
        #[cfg(feature = "test-with-testbed")]
        let conn = conn.or_else(|| super::testbed::maybe_open_sqlite_in_memory(&path_info));
...
async fn new_sqlite_executor_from_path(
    ...
    let pragma_options = PragmaOptions::default()
        .journal_mode(PragmaJournalMode::Wal)
...
impl LocalDatabase {
    ...
    pub async fn close(&self) {
        ...
        let conn = conn.and_then(|conn| {
            super::in_memory_mock::maybe_return_sqlite_in_memory_conn(&self.path_info, conn).err()
        });
        #[cfg(feature = "test-with-testbed")]
        let conn = conn.and_then(|conn| {
            super::testbed::maybe_return_sqlite_in_memory_conn(&self.path_info, conn).err()
        });
        drop(conn);
    }
