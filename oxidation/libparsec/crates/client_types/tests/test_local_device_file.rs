...
fn password_protected_device_file(alice: &Device) {
    ...
    let expected = DeviceFile::Password {
        ciphertext: ByteBuf::from(hex!(
        ...
        salt: ByteBuf::from(hex!("2ae6167f0f7472b8565c390df3af4a8b")),
    };

    let file_device = DeviceFile::load(&filedata).unwrap();
...
fn recovery_device_file(alice: &Device) {
    ...
    let expected = DeviceFile::Recovery {
        ciphertext: ByteBuf::from(hex!(
