...
///
/// The authentication is made by signing the request with a private key that the server is able to verify using the shared public key.
///
/// A request will typically have the following headers append to allow the server validate the request:
///
/// ```text
...
/// ```
///
/// The signature is generated by appending the `author` (in base64) + `timestamp` + `body` in that order.
...
impl AuthenticatedCmds {
    ...
    /// Prepare a new request, the body will be add to the Request using [RequestBuilder::body]
    fn prepare_request(&self, body: Vec<u8>) -> RequestBuilder {
        ...
        authorization_headers.insert(
            "Authorization",
            HeaderValue::from_static("PARSEC-SIGN-ED25519"),
        );
        authorization_headers.insert(
            "Author",
