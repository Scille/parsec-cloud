...
fn serde_ping_req() {
    ...
    let expected = PingReq {
        cmd: "ping".to_owned(),
        ping: "ping".to_owned(),
    };

    let schema = PingReq::load(&data).unwrap();
    ...
    let schema2 = PingReq::load(&data2).unwrap();
...
fn serde_ping_rep() {
    ...
    let expected = PingRep::Ok {
        pong: "pong".to_owned(),
    };

    let schema = PingRep::load(&data).unwrap();
    ...
    let data2 = schema.dump();
    // Note we cannot just compare with `data` due to signature and keys order
    let schema2 = PingRep::load(&data2).unwrap();
    assert_eq!(schema2, expected);
}
