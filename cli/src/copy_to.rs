...
use libparsec::{FsPath, OpenOptions, VlobID};
...
pub async fn copy_to(args: CopyTo) -> anyhow::Result<()> {
    let CopyTo {
        src,
        dest,
        workspace_id,
    ...
    loop {
        let bytes_read = buf_file.read(&mut buffer).await?;
