...
def list_dependencies_from_file(
    ...
    log = logger.getChild("listing")
    deps = set()
    with open(filepath, "rb") as file:
        log.debug(f"Loading {filepath} content")
        lock = loader(file)
...
def list_javascript_deps(versions: dict[str, str]) -> set[Dependency]:
    ...
    log = logger.getChild("list-javascript")
    deps = set([Dependency("nodejs", versions["node"], DependencyType.Javascript)])
    ...
    if deps_not_found:
        log.warning(f"Some dependencies where not found in lock file: {', '.join(deps_not_found)}")
    return deps
...
def list_rust_deps(_versions: dict[str, str]) -> set[Dependency]:
    ...
    log = logger.getChild("list-rust")
    with open(RUST_TOOLCHAIN_FILE, "rb") as toolchain:
        log.debug(f"Loading {RUST_TOOLCHAIN_FILE} content")
        data = tomllib.load(toolchain)
        version = str(data["toolchain"]["channel"])
        deps = set([Dependency("rust", version, DependencyType.Rust)])
    ...
    if deps_from_lock.not_found:
        log.warning(
            f"Some dependencies where not found in lock file: {', '.join(deps_from_lock.not_found)}"
        )
...
def list_python_deps(versions: dict[str, str]) -> set[Dependency]:
    ...
    log = logger.getChild("list-python")
    deps = set(
        [
