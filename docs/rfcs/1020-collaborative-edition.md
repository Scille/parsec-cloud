...
When the writers list change, we need to rotate the session key.

The rotation is performed by the users currently using the session, everyone will try to generate and shared a new manifest key,
...
- The session key cannot be known by other writer ahead of time
