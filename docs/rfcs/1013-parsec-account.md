...
```jinja
PARSEC-HMAC-BLAKE2B.{{ id }}.{{ base64(account_email) }}.{{ timestamp }}.{{ signature }}
```
...
content = \text{"PARSEC-HMAC-BLAKE2B"} \Vert auth_id \Vert base64(email) \Vert timestamp \Vert body\_sha256 \\
