...
On top of that, a user with profile OUTSIDER is not allowed to self-promote.

In the unlikely event where a workspace requiring self-promotion ends up with
only OUTSIDERs users, then an organization ADMIN should first promote one of
them to a STANDARD profile.
The newly STANDARD user can then proceed to the self-promotion, then typically
share the workspace to a more legitimate user and then be reverted back into
OUTSIDER with non-OWNER access to the workspace.
...
### 4.1 - No new certificate needed

The self-promotion uses the existing `RealmRoleCertificate`

### 4.2 - Update `LocalUserManifest` manifest

`LocalUserManifest` has to be modified to include this per-workspace self-promotion allowed status:
...
> the certificates.
>
> So adding this new `can_self_promote_to_owner` field is not strictly needed: instead the
> status could be computed from the certificates lazily (i.e. each time `Client::list_workspaces()`
> is called).
>
