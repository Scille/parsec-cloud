...
- add a `key_index` field (with type `IndexInt`). The server uses this to ensure
...
>
> However this doesn't take into account that a key that doesn't match it key
...
1. Invalid data: bad signature, bad encryption, bad serialization
2. Additional keys
3. Missing keys
...
3. For `RealmRoleCertificate`, if the role is None and we are owner of the realm:
    1. Wait a random time between 30 and 60 seconds (and reset the wait if we were
       already waiting for this realm)
    2. Call `realm_rotate_key`
4. For `RealmKeyRotationCertificate` cancel the wait from step 3.1 if any
