...
In regular synchronous enrollment, the two peers are designed as claimer
(the one that wants to be enrolled) and greeter (the one that does the enrollment).

However this term is changed to submitter & accepter in the asynchronous enrollment.
This is notably to reflect two fondamental differences:

- All administrator in the organization can deal with any given asynchronous enrollment (while
...
[
    {
        ...
        "req": {
            "fields": [
                ...
                },
                {
                    ...
                    // Must check `payload_signature` before loading it!
                    "name": "payload",
                    "type": "Bytes"
                },
                {
        ...
        "reps": [
            ...
            {
                // `payload` field doesn't correspond to a valid `AsyncEnrollmentSubmitPayload`
                "status": "invalid_payload"
            },
            {
                // `payload_signature` cannot be verified by the server
...
[
    {
        ...
        "req": {
            "fields": [
                ...
                {
                    // `AsyncEnrollmentAnswerPayload` in msgpack format.
                    // Must check `payload_signature` before loading it!
                    "name": "payload",
                    "type": "Bytes"
                },
        ...
        "reps": [
            ...
            {
                // `payload` field doesn't correspond to a valid `AsyncEnrollmentAnswerPayload`
                "status": "invalid_payload"
            },
            {
                // `payload_signature` cannot be verified by the server
...
```json5
{
    "label": "AsyncEnrollmentAnswerPayload",
    "type": "async_enrollment_answer_payload",
    "other_fields": [
        {
            "name": "user_id",
            "type": "UserID"
...
```json5
{
    "label": "AsynEnrollmentLocalPending",
    ...
    "other_fields": [
        ...
        {
            // To-be-created device's `VerifyKey` encrypted with the secret key from the `encrypted_key` field.
            "name": "ciphertext_signing_key",
            "type": "Bytes"
        }
        {
            // To-be-created user's `PrivateKey` encrypted with the secret key from the `encrypted_key` field.
...
Finally the accepter also signs his answer payload that has to be then validated
by the submitter.
