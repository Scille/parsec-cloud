...
### 2.2 - Workflow
 ...
 9. Bob sends to the Parsec server the accept payload along with its signature. This is similar
    to step 5 (but in reverse order).
10. Alice checks the Parsec server periodically (polling) until she obtains a response for
    initial request (either accepted with an accept payload or rejected/cancelled).
...
[
    {
        ...
        "reps": [
            ...
            },
            {
                ...
                // - Trustchain is technically valid, but it relies on a X509 root
                //   certificate that is not among the ones trusted by the server.
                // - The trustchain is too long (longer than 10 elements).
                // - The trustchain contains unneeded intermediate certificates.
...
```json5
{
    ...
    "other_fields": [
        {
            // Serialized `AsyncEnrollmentLocalPendingCleartextContent`
            // It contains all the fields that shouldn't be encrypted (typically
            // useful to list the pending enrollments without requiring authentication).
            "name": "cleartext_content",
            "type": "Bytes"
        },
        {
            // `HashDigest` (i.e. sha256 hash) of the `cleartext_content` field,
            // encrypted with the ciphertext key (i.e. a `SecretKey`).
            // This is used to ensure the cleartext data hasn't been tempered.
            "name": "ciphertext_cleartext_content_digest",
            "type": "Bytes"
        },
        {
...
```json5
{
    "label": "AsyncEnrollmentLocalPendingCleartextContent",
...
In PKI mode, the submitter signs his submit payload using his personal PKI's signing key, then
provides it along with his personal X509 certificate (and any intermediary X509 certificates
needed).

This way submit payload validation is done by:

1. Validating the X509 certificate chain
2. Ensuring the X509 root certificate  at the top of the chain is expected (see below).
...
- Server : here the allowed X509 root certificates are configured at startup.

> [!NOTE]
>
> - Obviously the X509 root certificate is never transmitted between clients,
>   it is instead obtained from the clients's OS certificates store.
...
at two levels: first in the server, then by the submitter.
...
1. Alice authenticates using OIDC, this provides a token that will be used to
   ...
   In OpenBAO, multiple OIDC authentications can link into the same entity (think
   of a user authenticating with different identity providers).
   Each OIDC authentication correspond to an entity alias in OpenBao, and this
   alias also store the email of the user (as transmitted to OpenBao by the
   identity provider).
2. Alice creates in OpenBao a signing key for its entity ID if it doesn't
   already exist.
3. Alice asks OpenBao to sign the payload using the signing key.
4. Alice transmits the payload, its signature and its entity ID to Bob.
5. Bob authenticate using OIDC.
6. Bob asks OpenBao to verify that the signature correspond to the payload signed
   with the entity ID provided by Alice.
7. Bob ask OpenBao to get the emails associated with the entity ID. Bob can then
   check that the email in the submit payload is among them.

> [!NOTE]
>
> - Only Alice is allowed to create and use the signing key for her entity ID
> - Any authenticated user is able to verify a payload signed with any key.
> - Any authenticated user is able to obtain the emails from a entity ID. This is
>   required to link the identity with the signed payload.
>   Note we don't consider it to be an oracle since the entity ID is a UUID,
>   so realistically only known entity ID can be queried.
> - An alternative solution would be to use the entity alias ID (instead of
>   the entity ID). This has the benefit of providing a single email for a given
>   signature, and `/identity/entity-alias/id/<entity_alias_id>` leaks less info.
>   However it has also two drawbacks: 1. it requires passing the entity alias ID
>   which is not convenient to obtain (typically it is not returned during
>   authentication), and 2. it requires a per-OIDC provider OpenBao policy
>   configuration which more complex and error-prone.
...
```python
# Step 1 omitted
# Alice entity ID is `217b7a03-b4d0-aff6-eaa8-4e1aa0573193`

# Step 2: Create key
POST https://openbao-dev.parsec.cloud/v1/transit/keys/user-217b7a03-b4d0-aff6-eaa8-4e1aa0573193
Headers: {"X-Vault-Token": "s.XXX"}
<<< status: 200 (also 200 if key already exists)
<<< {'request_id': '1cfb7461-5b77-333f-b952-21f34149267f', 'lease_id': '', 'renewable': False, 'lease_duration': 0, 'data': {'allow_plaintext_backup': False, 'auto_rotate_period': 0, 'deletion_allowed': False, 'derived': False, 'exportable': False, 'imported_key': False, 'keys': {'1': {'certificate_chain': '', 'creation_time': '2025-12-01T15:50:33.606080602Z', 'name': 'ed25519', 'public_key': 'ig9jt4uLZj4AxYSqdS3XkyymPamEijV4JaNHAfa1yUY='}}, 'latest_version': 1, 'min_available_version': 0, 'min_decryption_version': 1, 'min_encryption_version': 0, 'name': 'user-217b7a03-b4d0-aff6-eaa8-4e1aa0573193', 'soft_deleted': False, 'supports_decryption': False, 'supports_derivation': True, 'supports_encryption': False, 'supports_signing': True, 'type': 'ed25519'}, 'wrap_info': None, 'warnings': None, 'auth': None}

# Step 3: Sign payload
POST https://openbao-dev.parsec.cloud/v1/transit/sign/user-217b7a03-b4d0-aff6-eaa8-4e1aa0573193
Headers: {"X-Vault-Token": "s.XXX", "Content-Type": "application/json"}
Body: {"input": "<base 64 data>"}
<<< status: 200
...
POST https://openbao-dev.parsec.cloud/v1/transit/verify/user-217b7a03-b4d0-aff6-eaa8-4e1aa0573193
Headers: {"X-Vault-Token": "s.YYY", "Content-Type": "application/json"}
Body: {"input": "<base 64 data>", "signature": "vault:v1:C4jRZx+ymLou26tN8Q2KDy46dA47W7x/MH6nuEZVqdz+H0RvoaFbQUAHceBKhB+Qow2qXAXiRFAFWKGPZU93CQ=="}
<<< status: 200
...
# Step 7: Bob get Alice's emails from its entity ID
GET https://openbao-dev.parsec.cloud/v1/identity/entity/id/217b7a03-b4d0-aff6-eaa8-4e1aa0573193
Headers: {"X-Vault-Token": "s.YYY"}
<<< status: 200
<<< {'request_id': '6ad2d731-0ab7-f84e-d07d-a24543cff597', 'lease_id': '', 'renewable': False, 'lease_duration': 0, 'data': {'aliases': [{'canonical_id': '217b7a03-b4d0-aff6-eaa8-4e1aa0573193', 'creation_time': '2025-12-01T15:09:53Z', 'custom_metadata': None, 'id': 'd42286c0-a41d-bf0f-7dab-c9c27a0f0a58', 'last_update_time': '2025-12-01T15:09:53Z', 'local': False, 'merged_from_canonical_ids': None, 'metadata': {'role': 'default'}, 'mount_accessor': 'auth_oidc_e99f8297', 'mount_path': 'auth/oidc/', 'mount_type': 'oidc', 'name': 'test@fia1.fr'}], 'creation_time': '2025-12-01T15:09:53Z', 'direct_group_ids': [], 'disabled': False, 'group_ids': [], 'id': '217b7a03-b4d0-aff6-eaa8-4e1aa0573193', 'inherited_group_ids': [], 'last_update_time': '2025-12-01T15:09:53Z', 'merged_entity_ids': None, 'metadata': None, 'name': 'entity_46f449cb.root', 'namespace_id': 'root', 'policies': []}, 'wrap_info': None, 'warnings': None, 'auth': None}
```
...
# Other user gets info from the entity that signed a message to get its emails
