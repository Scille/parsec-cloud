...
This RFC introduces a **self-promotion** mechanism allowing the highest-ranked remaining user to claim the OWNER role.
...
3. When the user triggers that action, a confirmation dialog explains the situation
   ("All owners of this workspace have been revoked. As the highest-ranked remaining member,
...
- Revoking all OWNERs in a workspace is a unlikely event, so there should be ample time for all clients to update to a newer version of Parsec.
- The issue is present only if some clients use the newer and some others the older version. In such case it should be easy enough
...
> simplify the usage of this API (intent in the function name, more precise
> error variants).
