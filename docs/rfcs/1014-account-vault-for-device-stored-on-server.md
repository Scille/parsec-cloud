...
RFC 1014 quickly describe the Account Vault system, this RFC goes more in
detail regarding the lifecycle and the involved APIs.
...
we don't question de quality of pseudorandom-number-generator).
...
> Note: The format of this structure is inspired by the `RealmKeysBundleAccess` structure.
        ...
        "fields": [
            {
                ...
                // Typically for auto-enrollment device: `hash( 'AUTO_ENROLLMENT_DEVICE' + <organization ID> + <device ID>)`
                "type": "Bytes"
            },
            {
                "name": "item",
...
>   So the only concern would be on equivalent items (e.g. two auto-enrollment devices for
...
>   all items just to display the user the list of available auto-enrollment devices.
    ...
    "reps": [
        {
            ...
            "fields": [
                ...
                {
                    "name": "items",
                    // List of serialized `AccountVaultItem`
                    "type": "List<Bytes>",
                }
            ]
        }
