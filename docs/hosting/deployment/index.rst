...
3. Install ``parsec-server``
  ...
  .. code-block:: bash
    ...
    python -m pip install 'parsec-cloud==3.4.1-a.0+dev'
...
We recommend that connection to the service are using a TLS layer.
...
We recommend that when user directly connect to the server (without using a :ref:`reverse proxy<behind_a_proxy>`)
