...
We provide a detailed explanation of the required configuration below.
But if you just want to deploy a Parsec server locally you can do it with the following commands:
...
.. note::
   ...
   Those variables are used to configured the ``rustfs`` service that provide the object-storage API when using the ``docker-compose`` approach.
...
#. Install Parsec Server:
   ...
   .. code-block:: bash
     ...
     python -m pip install 'parsec-cloud==3.9.4-a.0.dev.20701+72e13e8'
