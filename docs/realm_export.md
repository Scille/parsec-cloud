...
parsec-cli organization bootstrap --addr=$BOOSTRAP_TOKEN --device-label='John PC' --label='John Doe' --email=john.doe@example.com --sequester-key=./sequester_authority_public.pem
```

> Note:
>
> `$BOOSTRAP_TOKEN` is the token obtained during the organization create step.
